...
export const WiredSelectorWithVariableView: FC<WiredSelectorWithVariableViewProps> = ({ selectorTarget }) => {
    ...
    const resolvedMainEntries = useMemo(() => {
        ...
        if (flattenWiredVariablePickerEntries(mainEntries).some((entry) => entry.token === variableToken))
            return mainEntries;
    ...
    const selectedMainEntry = useMemo(
        () =>
            flattenWiredVariablePickerEntries(resolvedMainEntries).find((entry) => entry.token === variableToken) ||
            null,
    ...
    const resolvedReferenceEntries = useMemo(() => {
        ...
        if (flattenWiredVariablePickerEntries(referenceEntries).some((entry) => entry.token === referenceVariableToken))
    ...
    const requiresFurni = referenceSelectionEnabled
    ...
    const referenceSourceOptions = useMemo(() => {
        if (referenceTargetType === 'furni')
            return resolveSourceOptions(orderedFurniSources, selectedReferenceSourceValue, orderedFurniSources);
    ...
    const save = () => {
        ...
        setReferenceFurniIds(nextReferenceFurniIds);
        setStringParam(
            `${variableToken || ''}\t${selectByValue && referenceMode === 'variable' ? referenceVariableToken : ''}`,
        );
        setIntParams([
            selectByValue ? 1 : 0,
            comparison,
                    ...
                    {selectByValue && referenceMode === 'variable' && (
                        <>
                            ...
                            <WiredFurniSelectionSourceRow
                                ...
                                onChange={(value) => {
                                    if (referenceTargetType === 'furni') {
                                        if (referenceFurniSource === SOURCE_SECONDARY_SELECTED)
                                            setReferenceFurniIds([...furniIds]);

...
export const WiredActionGiveRewardView: FC<{}> = (props) => {
    ...
    const updateReward = (index: number, updater: (reward: RewardEntry) => RewardEntry) => {
        setRewards((prevValue) =>
            prevValue.map((reward, rewardIndex) => (rewardIndex === index ? updater(reward) : reward)),
        );
    };
    ...
    const save = () => {
        ...
        for (const reward of rewards) {
            ...
            const probability = Math.max(
                0,
                Number.isFinite(reward.probability) ? reward.probability : DEFAULT_PROBABILITY,
            );
            const rewardCode = (() => {
            ...
            const rewardsString = [
                reward.rewardType === 'badge' ? '0' : '1',
                rewardCode,
                (uniqueRewards ? DEFAULT_PROBABILITY : probability).toString(),
            ];
    ...
    return (
        ...
        >
            <div className="flex items-center gap-1">
                ...
                <Text>
                    {LocalizeText(
                        'wiredfurni.params.prizelimit',
                        ['amount'],
                        [limitEnabled ? rewardsLimit.toString() : ''],
                    )}
                </Text>
            </div>
            {!limitEnabled && (
            ...
            {limitEnabled && (
                <Slider max={1000} min={1} value={rewardsLimit} onChange={(event) => setRewardsLimit(event)} />
            )}
                ...
                {rewards &&
                    rewards.map((reward, index) => {
                        const rewardTypeOptions =
                            reward.rewardType === 'respect' ? REWARD_TYPES : SELECTABLE_REWARD_TYPES;
                                ...
                                {reward.rewardType === 'points' ? (
                                    <NitroInput
                                        ...
                                        onChange={(event) =>
                                            updateReward(index, (prevValue) => ({
                                                ...prevValue,
                                                pointsType: Number(event.target.value),
                                            }))
                                        }
                                    />
                                ) : (
            ...
            <Text center small className="p-1 rounded bg-muted">
                Extra Currency uses Amount as the quantity and Currency Type as the purse type id. Example: amount 200 +
                type 105.
            </Text>
        </WiredActionBaseView>
    );

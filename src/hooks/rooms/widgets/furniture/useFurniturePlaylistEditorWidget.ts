...
const useFurniturePlaylistEditorWidgetState = () => {
    ...
    const addToPlaylist = useCallback(
        (diskId: number, slotNumber: number) => SendMessageComposer(new AddJukeboxDiskComposer(diskId, slotNumber)),
        [],
    );

    const removeFromPlaylist = useCallback(
        (slotNumber: number) => SendMessageComposer(new RemoveJukeboxDiskComposer(slotNumber)),
        [],
    );

    const togglePlayPause = useCallback(
        (furniId: number, position: number) => SendMessageComposer(new FurnitureMultiStateComposer(furniId, position)),
        [],
    );
    ...
    useNitroEvent<SongDiskInventoryReceivedEvent>(
        SongDiskInventoryReceivedEvent.SDIR_SONG_DISK_INVENTORY_RECEIVENT_EVENT,
        (event) => {

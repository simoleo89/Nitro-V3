...
export const UserAccountSettingsView: FC = () => {
    ...
    const submitPasswordChange = async () => {
        ...
        if (newPassword.length < MIN_PASSWORD_LENGTH) {
            setFeedback({
                kind: 'error',
                message: LocalizeText('usersettings.error.password_min', ['count'], [MIN_PASSWORD_LENGTH.toString()]),
            });
            return;
        }
        ...
        if (newUsername.length < MIN_USERNAME_LENGTH || newUsername.length > MAX_USERNAME_LENGTH) {
            setFeedback({
                ...
                message: LocalizeText(
                    'usersettings.error.username_length',
                    ['min', 'max'],
                    [MIN_USERNAME_LENGTH.toString(), MAX_USERNAME_LENGTH.toString()],
                ),
            });
            return;
        }
    ...
    return (
        <NitroCardView
            className="user-account-settings-window w-[360px]"
            theme="primary-slim"
                    ...
                    >
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            ...
                            <div className="relative flex items-center">
                                ...
                                <button
                                    ...
                                    aria-label={
                                        showCurrent
                                            ? LocalizeText('usersettings.aria.hide_password')
                                            : LocalizeText('usersettings.aria.show_password')
                                    }
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            ...
                            <Button variant="success" disabled={submitting} onClick={() => submitPasswordChange()}>
                                {submitting
                                    ? LocalizeText('usersettings.btn.saving')
                                    : LocalizeText('usersettings.btn.save_password')}
                            </Button>
                        </div>
                    </div>
                )}
                    ...
                    >
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            ...
                            <Button variant="success" disabled={submitting} onClick={() => submitEmailChange()}>
                                {submitting
                                    ? LocalizeText('usersettings.btn.saving')
                                    : LocalizeText('usersettings.btn.save_email')}
                            </Button>
                        </div>
                    </div>
                )}
                    ...
                    >
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            ...
                            <Button variant="warning" disabled={submitting} onClick={() => submitUsernameChange()}>
                                {submitting
                                    ? LocalizeText('usersettings.btn.renaming')
                                    : LocalizeText('usersettings.btn.rename')}
                            </Button>
                        </div>
                    </div>
                )}

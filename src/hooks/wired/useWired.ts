...
const useWiredState = () => {
    ...
    const saveWired = () => {
        const save = (trigger: Triggerable) => {
            ...
            if (trigger instanceof WiredActionDefinition) {
                ...
                SendMessageComposer(
                    new UpdateTriggerMessageComposer(
                        trigger.id,
                        intParams,
                        stringParam,
                        furniIds,
                ...
                SendMessageComposer(
                    new UpdateConditionMessageComposer(
                        trigger.id,
                        intParams,
                        stringParam,
                        furniIds,
    ...
    const selectObjectForWired = (objectId: number, category: number) => {
        ...
        const isAllowedInteraction = (furniData: any): boolean => {
            ...
            return comparableNames.some((value) =>
                allowedInteractionTypes.some((type) => !!type && matchesAllowedPattern(value, type)),
            );
        };
        ...
        if (selectByType && category === RoomObjectCategory.FLOOR) {
            ...
            setFurniIds((prevValue) => {
                ...
                for (const obj of allFloorObjects) {
                    ...
                    const furniLineMatch =
                        matchFurniLine && matchFurniLine.length > 0 && fd.furniLine === matchFurniLine;

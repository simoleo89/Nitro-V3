...
const useAvatarEditorState = () => {
    ...
    const randomizeCurrentFigure = useCallback(
        (ignoredSets: string[] = []) => {
            ...
            const getRandomSetTypes = (requiredSets: string[], options: string[]) => {
                ...
                return [
                    ...requiredSets,
                    ...Randomizer.getRandomElements(options, Randomizer.getRandomNumber(options.length) + 1),
                ];
            };

            const requiredSets = getRandomSetTypes(
                structure.getMandatorySetTypeIds(gender, GetClubMemberLevel()),
                AvatarFigurePartType.FIGURE_SETS,
            );
            ...
            for (const type of requiredSets) {
                ...
                if (selectedSet.isColorable) {
                    selectedColors = getRandomColors(
                        structure.figureData.getPalette(setType.paletteID),
                        selectedSet,
                        GetClubMemberLevel(),
                    ).map((color) => color.id);
                }
    ...
    useEffect(() => {
        ...
        const buildCategory = (setType: string, buildMode: string = buildModeDefault) => {
            ...
            let mandatorySetIds: string[] = GetAvatarRenderManager().getMandatoryAvatarPartSetIds(
                gender,
                GetClubMemberLevel(),
            );
            ...
            for (let i = partSets.length; i >= 0; i--) {
                ...
                if (
                    ...
                    (partSet.gender !== gender && partSet.gender !== AvatarFigurePartType.UNISEX)
                )
                    continue;

                const isNftPartSet =
                    nftFigureSetIds.size > 0
        ...
        newAvatarModels[AvatarEditorFigureCategory.GENERIC] = [AvatarFigurePartType.HEAD].map((setType) =>
            buildCategory(setType, buildModeDefault),
        );
        ...
        newAvatarModels[AvatarEditorFigureCategory.LEGS] = [
            AvatarFigurePartType.LEGS,
            AvatarFigurePartType.SHOES,
            AvatarFigurePartType.WAIST_ACCESSORY,
        ].map((setType) => buildCategory(setType, buildModeDefault));
        newAvatarModels[AvatarEditorFigureCategory.PETS] = [AvatarFigurePartType.PET]
            .map((setType) => buildCategory(setType))
            .filter(Boolean);
        newAvatarModels[AvatarEditorFigureCategory.MISC] = [AvatarFigurePartType.MISC]
            .map((setType) => buildCategory(setType))
            .filter(Boolean);

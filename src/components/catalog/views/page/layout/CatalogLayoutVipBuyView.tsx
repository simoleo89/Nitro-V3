    ...
    const getPurchaseButton = useCallback(() => {
        ...
        if (pendingOffer.priceActivityPoints > getCurrencyAmount(pendingOffer.priceActivityPointsType)) {
            return (
                <Button fullWidth variant="danger">
                    {LocalizeText(
                        'catalog.alert.notenough.activitypoints.title.' + pendingOffer.priceActivityPointsType,
                    )}
                </Button>
            );
        }
        ...
        <Grid>
                                ...
                                >
                                    <div className="vip-offer-header flex items-center gap-2 pb-1.5 border-b border-dashed border-[#b48a18]">
                                        <span className="vip-offer-banner inline-flex items-center justify-center shrink-0 w-[34px] h-[20px]">
                                            <i
                                                className="nitro-icon icon-hc-banner"
                                                style={{
                                                    width: '34px',
                                                    height: '20px',
            ...
            <Column overflow="hidden" size={5}>
                <Column center fullHeight overflow="hidden">
                    ...
                    <Text
                        center
                        dangerouslySetInnerHTML={{ __html: SanitizeHtml(getSubscriptionDetails) }}
                        overflow="auto"
                    />
                </Column>
                {pendingOffer && (
                    <Column fullWidth grow justifyContent="end">
                        ...
                        {pendingOffer.giftable && (
                            <Column className="mt-1" gap={1}>
                                ...
                                {giftMode && isSelfGift && (
                                    <Text className="text-[#b00020] text-xs">
                                        {LocalizeText('catalog.gift_wrapping.cannot_send_to_self')}
                                    </Text>
                                )}
                                ...
                                {giftSuccess && (
                                    <Text className="text-[#1f7a1f] text-sm font-bold">
                                        {LocalizeText('catalog.gift_wrapping.gift_sent')}
                                    </Text>
                                )}
                            </Column>
                        )}

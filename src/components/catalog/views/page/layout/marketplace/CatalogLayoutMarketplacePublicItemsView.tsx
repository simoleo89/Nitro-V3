...
export const CatalogLayoutMarketplacePublicItemsView: FC<CatalogLayoutMarketplacePublicItemsViewProps> = (props) => {
    ...
    const [lastSearch, setLastSearch] = useState<IMarketplaceSearchOptions>({
        minPrice: -1,
        maxPrice: -1,
        query: '',
        type: 3,
    ...
    const requestOffers = useCallback((options: IMarketplaceSearchOptions) => {
        ...
        SendMessageComposer(
            new GetMarketplaceOffersMessageComposer(options.minPrice, options.maxPrice, options.query, options.type),
        );
    }, []);
    ...
    return (
        <>
            <div className="relative inline-flex align-middle">
                <Button
                    active={searchType === MarketplaceSearchType.BY_ACTIVITY}
                    onClick={() => setSearchType(MarketplaceSearchType.BY_ACTIVITY)}
                >
                    {LocalizeText('catalog.marketplace.search_by_activity')}
                </Button>
                <Button
                    active={searchType === MarketplaceSearchType.BY_VALUE}
                    onClick={() => setSearchType(MarketplaceSearchType.BY_VALUE)}
                >
                    {LocalizeText('catalog.marketplace.search_by_value')}
                </Button>
                <Button
                    active={searchType === MarketplaceSearchType.ADVANCED}
                    onClick={() => setSearchType(MarketplaceSearchType.ADVANCED)}
                >

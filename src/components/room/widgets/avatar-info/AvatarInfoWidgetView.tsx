...
export const AvatarInfoWidgetView: FC = (props) => {
    ...
    useUiEvent<RoomWidgetUpdateRentableBotChatEvent>(RoomWidgetUpdateRentableBotChatEvent.UPDATE_CHAT, (event) =>
    ...
    const getMenuView = () => {
        ...
        if (activeNameBubble)
            return <AvatarInfoWidgetNameView nameInfo={activeNameBubble} onClose={() => setActiveNameBubble(null)} />;
        ...
        if (avatarInfo) {
            switch (avatarInfo.type) {
                ...
                case AvatarInfoPet.PET_INFO: {
                    ...
                    if (info.isOwner)
                        return <AvatarInfoWidgetOwnPetView avatarInfo={info} onClose={() => setAvatarInfo(null)} />;
            ...
            case AvatarInfoFurni.FURNI:
                if (isTouchLayout && !isDecorating) {
                    ...
                    if (!mobileFurniDetailsOpen) {
                        return (
                            <button
                                className="nitro-mobile-furni-infostand-trigger"
                                type="button"
                                onClick={() => setMobileFurniDetailsOpen(true)}
                            >
                                <LayoutFurniIconImageView
                                    productType={info.productType}
                                    productClassId={info.spriteId}
                                />
                            </button>
                        );
                    }
                        ...
                        return (
                            ...
                            >
                                <div className="nitro-mobile-user-infostand-avatar">
                                    <img
                                        className="nitro-mobile-user-infostand-avatar-image"
                                        src={avatarHeadUrl}
                                        alt={info.name}
                                        draggable={false}
                                    />
    ...
    return (
        <>
            ...
            {avatarInfo && (
                <Column
                    alignItems="end"
                    className="absolute right-[10px] bottom-[65px] pointer-events-none z-30 text-white"
                >
                    {getInfostandView()}
                </Column>
            )}
            {nameBubbles.length > 0 &&
                nameBubbles.map((name, index) => (
                    <AvatarInfoWidgetNameView key={index} nameInfo={name} onClose={() => removeNameBubble(index)} />
                ))}

...
export const WiredVariablePicker: FC<WiredVariablePickerProps> = (props) => {
    ...
    const activeParent =
        filteredEntries.find((entry) => entry.token === activeParentToken && entry.children?.length) || null;
    const portalTarget =
    ...
    const rememberSelection = (token: string) => {
        ...
        const nextRecentTokens = [token, ...recentTokens.filter((currentToken) => currentToken !== token)].slice(
            0,
            RECENT_PICKER_LIMIT,
        );
            ...
            >
                ...
                <div className="nitro-wired__variable-picker-search">
                    ...
                    {!!query.length && (
                        <button
                            type="button"
                            className="nitro-wired__variable-picker-clear"
                            onClick={() => setQuery('')}
                        >
    ...
    return (
        <div className="nitro-wired__variable-picker" ref={containerRef}>
            <button
                type="button"
                className="form-select form-select-sm nitro-wired__variable-picker-trigger"
                onClick={() => setIsOpen((value) => !value)}
            >
                <span className={selectedEntry ? '' : 'nitro-wired__variable-picker-placeholder'}>

...
export const CatalogLayoutTrophiesView: FC<CatalogLayoutProps> = (props) => {
    ...
    return (
        <div className="flex flex-col h-full gap-2">
            ...
            {currentOffer ? (
                <div
                    className="flex gap-0 bg-white rounded border-2 border-warning/40 shrink-0"
                    style={{ boxShadow: '0 0 8px rgba(255,193,7,0.15)' }}
                >
                    {/* Preview */}
                    <div
                        className="w-[120px] min-w-[120px] relative flex items-center justify-center border-r-2 border-warning/30"
                        style={{ background: 'linear-gradient(180deg, #fff9e6 0%, #fff3cc 100%)' }}
                    >
                    ...
                    <div className="flex flex-col flex-1 min-w-0 p-2 gap-1.5">
                        <div className="flex items-center gap-1.5">
                            ...
                            <Text className="text-[12px]! font-bold text-dark leading-tight">
                        ...
                        {adminMode && (
                            <div className="flex items-center gap-1 flex-wrap">
                                ...
                                <span className="text-[8px] font-mono text-white bg-primary px-1 py-px rounded">
                        ...
                        {!canPurchase && (
                            <span className="text-[9px] text-warning italic">
                                {LocalizeText('catalog.trophies.write.hint')}
                            </span>
                        )}
                ...
                <div className="flex items-start gap-3 p-2.5 bg-white rounded border-2 border-card-grid-item-border">
                    {!!page.localization.getImage(1) && (
                        <img
                            className="w-[50px] h-[50px] object-contain rounded shrink-0 mt-0.5"
            ...
            <div className="flex flex-col gap-1">
                <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] font-bold text-muted uppercase tracking-wider">
                        {LocalizeText('catalog.trophies.inscription')}
                    </span>
                    <span
                        className={`text-[9px] ml-auto ${trophyText.length > 180 ? 'text-danger font-bold' : 'text-muted'}`}
                    >
                        {trophyText.length}/200
                    </span>
                </div>
                <div className="relative">
                    <textarea
                        ...
                        style={{
                            background:
                                trophyText.length > 0 ? 'linear-gradient(180deg, #fffdf5 0%, #fff8e8 100%)' : '#fff',

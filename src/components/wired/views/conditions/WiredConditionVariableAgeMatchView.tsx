...
const buildCustomVariableEntries = (
    target: VariableTargetType,
...
export const WiredConditionVariableAgeMatchView: FC<{}> = () => {
    ...
    const variableEntries = useMemo(
        () => buildCustomVariableEntries(targetType, variableDefinitions),
    ...
    const requiresFurni =
        targetType === 'furni' && furniSource === SOURCE_SELECTED
    ...
    useEffect(() => {
        ...
        setCompareValue(
            intData.length > 1 && intData[1] === COMPARE_VALUE_UPDATED ? COMPARE_VALUE_UPDATED : COMPARE_VALUE_CREATED,
        );
        setComparison(
            intData.length > 2 && intData[2] === COMPARISON_HIGHER_THAN
                ? COMPARISON_HIGHER_THAN
        ...
        setFurniSource(
            intData.length > 6
                ? intData[6]
                : (trigger.selectedItems?.length ?? 0) > 0
                  ? SOURCE_SELECTED
                  : SOURCE_TRIGGER,
        );

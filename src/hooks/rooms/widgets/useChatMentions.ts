...
export const useChatMentions = (
    ...
    const mentionContext = useMemo(
        () => computeMentionContext(chatValue, commandSelectorVisible),
    ...
    const suggestions = useMemo<MentionSuggestion[]>(() => {
        ...
        return buildChatMentionSuggestions(
    ...
    const moveUp = useCallback(
        () => setSelectedIndex(safeIndex <= 0 ? suggestions.length - 1 : safeIndex - 1),
        [safeIndex, suggestions.length],
    );
    const moveDown = useCallback(
        () => setSelectedIndex(safeIndex >= suggestions.length - 1 ? 0 : safeIndex + 1),
        [safeIndex, suggestions.length],
    );

    ...
    return (
        <>
            <div className="flex justify-center py-3 rounded bg-success container-fluid">
                <img
                    className="my-music"
                    src={GetConfigurationValue('image.library.url') + 'playlist/title_mymusic.gif'}
                />
                    ...
                    {diskInventory &&
                        diskInventory.getKeys().map((key, index) => {
                            ...
                            return (
                                ...
                                >
                                    <div
                                        className="shrink-0 disk-image mb-n2"
                                        style={{ backgroundColor: GetDiskColor(songInfo?.songData) }}
                                    ></div>
                                    ...
                                    {selectedItem === index && (
                                        ...
                                        >
                                            <Button variant="light" onClick={(event) => previewSong(event, songId)}>
                                                <div
                                                    className={previewSongId === songId ? 'pause-btn' : 'preview-song'}
                                                />
                                            </Button>
            ...
            <img
                className="get-more"
                src={`${GetConfigurationValue('image.library.url')}playlist/background_get_more_music.gif`}
            />
        </>
    );
};

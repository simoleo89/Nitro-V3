...
export const RoomFilterWordsWidgetView: FC<{}> = (props) => {
    ...
    const processAction = (isAddingWord: boolean) => {
        ...
        SendMessageComposer(
            new UpdateRoomFilterMessageComposer(
                navigatorData.enteredGuestRoom.roomId,
                isAddingWord,
                isSelectingWord ? selectedWord : word,
            ),
        );
        setSelectedWord('');
        setWord('bobba');
    ...
    return (
        <NitroCardView className="nitro-guide-tool no-resize" theme="primary-slim">
            <NitroCardHeaderView
                headerText={LocalizeText('navigator.roomsettings.roomfilter')}
            ...
            <NitroCardContentView className="text-black">
                <Grid className="flex items-center gap-2 justify-end">
                    ...
                    />
                    <Button onClick={() => processAction(true)}>
                        {LocalizeText('navigator.roomsettings.roomfilter.addword')}
                    </Button>
                </Grid>
                <Column
                ...
                <Grid className="flex items-center gap-2 justify-end">
                    <Button
                        disabled={wordsFilter.length === 0 || !isSelectingWord}
                        variant="danger"

...
const PrefixPreview: FC<{
    ...
    className?: string;
    textSize?: string;
}> = ({ text, color, icon, effect = '', font = '', className = '', textSize = 'text-sm' }) => {
        ...
        <span className={`font-bold ${textSize} ${className}`} style={{ ...fontStyle, ...fxStyle }}>
            ...
            <span
                style={
                    hasMultiColor
                        ? { ...fontStyle, ...fxStyle }
                        : { ...fontStyle, ...fxStyle, color: colors[0] || '#FFFFFF' }
                }
            >
                {'{'}
                {hasMultiColor
...
const NickIconItemView: FC<{
    ...
    return (
        ...
        >
            {isActive && (
                <span className="absolute right-1 top-1 rounded bg-[#15954c] px-1 py-0.5 text-[8px] font-bold uppercase text-white">
                    Active
                </span>
            )}
            <div className="flex flex-col items-center gap-1">
                <img
                    ...
                    src={GetNickIconUrl(iconKey)}
                    alt={displayName || iconKey}
                />
                <span className="max-w-[90px] truncate text-center text-[11px] font-bold">
                    {displayName || iconKey}
                </span>
            </div>
        </div>
...
export const InventoryPrefixView: FC<{}> = () => {
    ...
    const selectedIconUrl = useMemo(
        () => (selectedNickIcon ? GetNickIconUrl(selectedNickIcon.iconKey) : ''),
        [selectedNickIcon],
    );
                                    ...
                                    <NitroButton
                                        ...
                                        onClick={() =>
                                            selectedPrefix.active
                                                ? deactivatePrefix()
                                                : activatePrefix(selectedPrefix.id)
                                        }
                                    >
                        ...
                        {!!selectedNickIcon && (
                            <div className="mt-2 flex flex-col gap-2">
                                ...
                                <Button
                                    disabled={false}
                                    onClick={() =>
                                        selectedNickIcon.active
                                            ? deactivateNickIcon()
                                            : activateNickIcon(selectedNickIcon.id)
                                    }
                                >

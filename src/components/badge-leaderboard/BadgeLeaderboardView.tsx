...
export const BadgeLeaderboardView: FC<{}> = (props) => {
    ...
    const rankedTotalPlayers = Math.max(
        (currentPage?.board?.totalPlayers || 0) - (viewerHasRankedScore ? 1 : 0),
        ...
        <div className="nitro-badge-leaderboard fixed inset-0 z-[100] flex items-center justify-center pointer-events-none">
            ...
            >
                <div
                    ...
                    style={
                        {
                            '--badge-leaderboard-frame': `url(${currentPage?.frame || frameLeaderboardTotal})`,
                        } as CSSProperties
                    }
                >
                    ...
                    >
                        <span
                            className="nitro-badge-leaderboard__close-icon"
                            style={{ backgroundImage: `url(${leaderboardButtonCloseSwf})` }}
                        />
                    </button>
                    <div className="nitro-badge-leaderboard__header">
                        ...
                        >
                            ...
                            <img
                                className="nitro-badge-leaderboard__header-arrow"
                                src={leaderboardDropdownOpener}
                                alt=""
                            />
                        </button>
                        {isCategoryMenuVisible && (
                            <div
                                className="nitro-badge-leaderboard__category-menu"
                                onPointerDown={(event) => event.stopPropagation()}
                            >
                    ...
                    <div className="nitro-badge-leaderboard__content">
                        {isLoading && !leaderboard && (
                            <div className="nitro-badge-leaderboard__state">{LocalizeText('generic.loading')}</div>
                        )}
                        ...
                        {currentPage && (
                            <>
                                <div className="nitro-badge-leaderboard__info-card">
                                    <img
                                        className="nitro-badge-leaderboard__info-icon"
                                        src={currentPage.emblem}
                                        alt=""
                                    />
                                ...
                                <Flex
                                    className="nitro-badge-leaderboard__footer"
                                ...
                                >
                                    ...
                                    <Column gap={0} alignItems="center">
                                        ...
                                        <Text
                                            ...
                                            small
                                        >{`${clampedEntryPageIndex + 1} / ${totalEntryPages}`}</Text>
                                    </Column>
                                    <button
                                        className="nitro-badge-leaderboard__nav-button is-next"
                                        disabled={clampedEntryPageIndex >= totalEntryPages - 1}
                                        onClick={() =>
                                            setEntryPageIndex((value) => Math.min(totalEntryPages - 1, value + 1))
                                        }
                                    >
...
const LeaderboardRow: FC<LeaderboardRowProps> = (props) => {
    ...
    const rankClassName =
        entry.rank === 1 ? 'is-rank-1' : entry.rank === 2 ? 'is-rank-2' : entry.rank === 3 ? 'is-rank-3' : '';

    return (
        <div
            className={`nitro-badge-leaderboard__row ${isCurrentUser ? 'is-current-user' : ''} ${rowIndex % 2 === 0 ? 'is-even' : 'is-odd'}`}
        >

...
export const WheelWinReveal: FC<WheelWinRevealProps> = ({ prize, onDismiss }) => {
    ...
    if (tier === 'none') {
        return (
            <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/35" onClick={onDismiss}>
                <div
                    className="rounded-xl bg-white px-6 py-4 text-center shadow-2xl"
                    style={{ animation: 'wheelPop .45s cubic-bezier(.18,.89,.32,1.28)' }}
                >
    ...
    return (
        ...
        >
            ...
            {isRare && (
                <div className="text-sm font-black uppercase tracking-[0.2em] text-[#ffd34d] drop-shadow">
            ...
            <div
                ...
                style={{
                    animation: `wheelPop .5s cubic-bezier(.18,.89,.32,1.28)${isRare ? ', wheelGlow 1.4s ease-in-out .5s infinite' : ''}`,
                }}
            >
            ...
            {!!prize.label && (
                <div className="rounded-full bg-white/95 px-4 py-1 text-sm font-bold text-[#20313a] shadow">
                    {prize.label}
                </div>
            )}
        </div>

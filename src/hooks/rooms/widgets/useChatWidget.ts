...
const useChatWidgetState = () => {
    ...
    const buildTranslatedEntryPatch = useCallback(
        (originalText: string, translatedText: string, detectedLanguage: string, targetLanguage: string) => {
    ...
    const applyAsyncTranslation = useCallback(
        (
            ...
            setChatMessages((prevValue) => {
                ...
                if (bubble)
                    applyTranslationToBubble(bubble, originalText, translatedText, detectedLanguage, targetLanguage);
            ...
            updateChatEntry(
                chatEntryId,
                buildTranslatedEntryPatch(originalText, translatedText, detectedLanguage, targetLanguage),
            );
        },
    ...
    useNitroEvent<RoomSessionChatEvent>(RoomSessionChatEvent.CHAT_EVENT, async (event) => {
        ...
        switch (chatType) {
            case RoomSessionChatEvent.CHAT_TYPE_RESPECT:
                ...
                if (GetConfigurationValue('respect.options')['enabled'])
                    PlaySound(GetConfigurationValue('respect.options')['sound']);
        ...
        const outgoingTranslation =
            isTranslatableChatType && userData.webID === ownUserId ? consumeOutgoingTranslation(text) : null;

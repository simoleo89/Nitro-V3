...
export const WiredActionGiveVariableView: FC = () => {
    ...
    const resolvedVariableEntries = useMemo(() => {
        ...
        if (flattenWiredVariablePickerEntries(variableEntries).some((entry) => entry.token === selectedVariableToken))
    ...
    const selectedVariableDefinition = useMemo(
        () =>
            flattenWiredVariablePickerEntries(resolvedVariableEntries).find(
                (entry) => entry.token === selectedVariableToken,
            ) ?? null,
    ...
    const sourceOptions =
        selectedTargetType === 'user' ? orderedUserSources : selectedTargetType === 'furni' ? orderedFurniSources : [];
    ...
    const save = () => {
        ...
        setIntParams([
            ...
            Number.isFinite(parsedInitialValue) ? parsedInitialValue : 0,
            userSource,
            furniSource,
        ]);

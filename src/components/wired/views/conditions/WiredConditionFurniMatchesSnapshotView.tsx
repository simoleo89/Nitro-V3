...
export const WiredConditionFurniMatchesSnapshotView: FC<WiredConditionFurniMatchesSnapshotViewProps> = ({
    negative = false,
}) => {
    ...
    return (
                            ...
                            return (
                                <label key={value} className="flex items-center gap-1">
                                    <input
                                        ...
                                        name="snapshotQuantifier"
                                        type="radio"
                                        onChange={() => setQuantifier(value)}
                                    />
                                    <Text>
                                        {LocalizeText(
                                            `wiredfurni.params.quantifier.furni${negative ? '.neg' : ''}.${value}`,
                                        )}
                                    </Text>
                                </label>
                            );
        ...
        >
            <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1">
                    <input
                        checked={!!altitudeFlag}
                        className="form-check-input"
                        id="altitudeFlag"
                        type="checkbox"
                        onChange={(event) => setAltitudeFlag(event.target.checked ? 1 : 0)}
                    />

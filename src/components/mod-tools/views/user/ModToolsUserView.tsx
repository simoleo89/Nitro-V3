...
const StatCard: FC<StatCardProps> = ({ icon, label, value, tone = 'neutral' }) => {
    ...
    return (
        <div
            className={`flex flex-col items-center justify-center px-2 py-1.5 rounded border ${toneClasses} grow min-w-0`}
        >
...
const Field: FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
    <>
        ...
        <dd className="m-0 break-words font-medium">
            {value || value === 0 ? value : <span className="opacity-40">-</span>}
        </dd>
    </>
);
...
export const ModToolsUserView: FC<ModToolsUserViewProps> = (props) => {
    ...
    return (
        <>
            <NitroCardView
                className="nitro-mod-tools-user min-w-[420px] max-w-[480px]"
                theme="primary-slim"
                ...
                <NitroCardContentView className="text-black" gap={2}>
                    ...
                    <div className="flex flex-col gap-2 max-h-[300px] overflow-auto pr-1">
                        <Section title={LocalizeText('modtools.userinfo.section.account')}>
                            <Field
                                label={LocalizeText('modtools.userinfo.primaryEmailAddress')}
                            ...
                            />
                            <Field
                                label={LocalizeText('modtools.userinfo.userClassification')}
                        ...
                        <Section title={LocalizeText('modtools.userinfo.section.activity')}>
                            ...
                            />
                            <Field
                                label={LocalizeText('modtools.userinfo.lastPurchaseDate')}
                                value={userInfo.lastPurchaseDate}
                            />
                        </Section>
                        <Section title={LocalizeText('modtools.userinfo.section.sanctions')}>
                            <Field
                                label={LocalizeText('modtools.userinfo.abusiveCfhCount')}
                                value={userInfo.abusiveCfhCount}
                            />
                            <Field
                            ...
                            <Field
                                ...
                                value={userInfo.identityRelatedBanCount}
                            />
                        </Section>
                        <Section title={LocalizeText('modtools.userinfo.section.trading')}>
                            <Field
                                label={LocalizeText('modtools.userinfo.tradingExpiryDate')}
                    ...
                    <div className="grid grid-cols-2 gap-1.5 pt-1 border-t border-zinc-200">
                        <Button
                            gap={1}
                            variant="secondary"
                            onClick={() => CreateLinkEvent(`mod-tools/open-user-chatlog/${userId}`)}
                        >

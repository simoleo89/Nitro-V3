...
const BADGE_RARITY_COLORS: Record<
    string,
    { glow: string; pillBackground: string; pillBorder: string; pillText: string }
> = {
    common: {
        glow: 'rgba(148, 163, 184, 0.55)',
        pillBackground: 'rgba(71, 85, 105, 0.16)',
    ...
    epic: {
        ...
        pillBorder: 'rgba(147, 51, 234, 0.4)',
        pillText: '#7e22ce',
    },
    legendary: {
        glow: 'rgba(249, 115, 22, 0.76)',
        pillBackground: 'rgba(249, 115, 22, 0.16)',
        pillBorder: 'rgba(234, 88, 12, 0.4)',
        pillText: '#c2410c',
    },
    mythical: {
        glow: 'rgba(236, 72, 153, 0.76)',
        pillBackground: 'rgba(236, 72, 153, 0.15)',
        pillBorder: 'rgba(219, 39, 119, 0.4)',
        pillText: '#be185d',
    },
    unique: {
        glow: 'rgba(34, 197, 94, 0.76)',
        pillBackground: 'rgba(34, 197, 94, 0.14)',
        pillBorder: 'rgba(22, 163, 74, 0.4)',
        pillText: '#15803d',
    },
};
...
export const LayoutBadgeImageView: FC<LayoutBadgeImageViewProps> = (props) => {
    ...
    useEffect(() => {
        ...
        const onBadgeImageReadyEvent = async (event: BadgeImageReadyEvent) => {
            ...
            if (isGroup) {
                ...
                const badgeUrl = GetConfigurationValue<string>('badge.asset.url').replace(
                    '%badgename%',
                    badgeCode.toString(),
                );
                const img = new Image();
    ...
    const ownersText = badgeRarityStat
        ? LocalizeText('badge.owner_count', ['count'], [badgeRarityStat.ownerCount.toString()])
        : '';

    ...
    return (
        <>
            <NitroCardView className="nitro-friends" theme="primary-slim" uniqueKey="nitro-friends">
                <NitroCardHeaderView
                    headerText={LocalizeText('friendlist.friends')}
                    ...
                    <NitroCardAccordionView fullHeight overflow="hidden">
                        ...
                        >
                            <Flex className="friends-list-toolbar px-2 py-1" justifyContent="end">
                                ...
                                >
                                    {filteredOfflineFriends.length &&
                                    filteredOfflineFriends.every((friend) => selectedFriendsIds.indexOf(friend.id) >= 0)
                                        ? LocalizeText('friendlist.unselect_all')
                                        : LocalizeText('friendlist.select_all')}
                                </span>
                            </Flex>
                        ...
                        <FriendsListRequestView
                            headerText={LocalizeText('friendlist.tab.friendrequests') + ` (${requests.length})`}
                            isExpanded={true}
                        />
                    ...
                    {selectedFriendsIds && selectedFriendsIds.length > 0 && (
                        <Flex className="p-1" gap={1}>
                            ...
                            <Button
                                fullWidth
                                variant="danger"
                                onClick={(event) => setShowRemoveFriendsConfirmation(true)}
                            >
            ...
            {showCategoryManager && (
                <FriendsCategoryManagerView
                    categories={categories}
                    onCloseClick={() => setShowCategoryManager(false)}
                />
            )}
        </>
    );

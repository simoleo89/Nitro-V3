    ...
    const attemptDeleteBadge = () => {
        ...
        showConfirm(
            LocalizeText(
                'inventory.delete.confirm_delete.info',
                ['furniname', 'amount'],
                [LocalizeBadgeName(selectedBadgeCode), '1'],
            ),
        ...
        <div className="grid h-full grid-cols-12 gap-2">
            ...
            >
                ...
                <div className="flex items-center gap-1 text-xs">
                    ...
                    >
                        {LocalizeText('inventory.badges.tab.all') !== 'inventory.badges.tab.all'
                            ? LocalizeText('inventory.badges.tab.all')
                            : 'All'}{' '}
                        ({baseCodes.length})
                    </button>
                    <button
                    ...
                    >
                        {LocalizeText('inventory.badges.tab.custom') !== 'inventory.badges.tab.custom'
                            ? LocalizeText('inventory.badges.tab.custom')
                            : 'Custom'}{' '}
                        ({customCount})
                    </button>
                    <button
            ...
            <div className="flex flex-col justify-between col-span-5 overflow-auto">
                <div className="flex flex-col gap-2 overflow-hidden">
                    <span className="text-sm truncate min-h-[1.25rem] leading-5">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                {LocalizeText(
                                    isWearingBadge(selectedBadgeCode)
                                        ? 'inventory.badges.clearbadge'
                                        : 'inventory.badges.wearbadge',
                                )}
                            </NitroButton>
                            {isOwnCustomBadge(selectedBadgeCode) && (
                                <NitroButton
                                    className="p-1"
                                    title={
                                        LocalizeText('inventory.badges.edit') !== 'inventory.badges.edit'
                                            ? LocalizeText('inventory.badges.edit')
                                            : 'Edit'
                                    }
                            ...
                            {!isWearingBadge(selectedBadgeCode) && (
                                <NitroButton
                                    ...
                                    onClick={
                                        isOwnCustomBadge(selectedBadgeCode) ? handleDeleteCustom : attemptDeleteBadge
                                    }
                                >

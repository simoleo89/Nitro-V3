    ...
    try {
        ...
        if (configured && typeof configured === 'object') {
            clientModeCache = {
                ...
                plainConfigBaseUrl:
                    typeof configured.plainConfigBaseUrl === 'string' ? configured.plainConfigBaseUrl : '',
                plainGamedataBaseUrl:
                    typeof configured.plainGamedataBaseUrl === 'string' ? configured.plainGamedataBaseUrl : '',
            };
...
const readRequestBody = async (
    ...
    if (init?.body !== undefined) {
        ...
        if (ArrayBuffer.isView(init.body))
            return init.body.buffer.slice(init.body.byteOffset, init.body.byteOffset + init.body.byteLength);
...
const buildSecureApiEnvelope = (
    requestUrl: string,
    method: string,
    clearBody: ArrayBuffer | null,
): ArrayBuffer | null => {
...
export const installSecureFetch = (): void => {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        if (requestUrl.includes('/nitro-sec/file')) {
            if (!getClientMode().secureAssetsEnabled)
                return nativeFetch(mapSecureAssetRequestToPlainUrl(requestUrl), init);

            const method = init?.method || (input instanceof Request ? input.method : 'GET');
            const cacheKey = method.toUpperCase() === 'GET' ? normalizeSecureCacheKey(requestUrl) : null;

            if (cacheKey && secureResponseCache.has(cacheKey))
                return cloneCachedResponse(secureResponseCache.get(cacheKey));

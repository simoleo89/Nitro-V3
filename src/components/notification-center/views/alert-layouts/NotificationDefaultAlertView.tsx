...
export const NotificationDefaultAlertView: FC<NotificationDefaultAlertViewProps> = (props) => {
    ...
    const copyCommandToChatInput = (command: string) => {
        ...
        DispatchUiEvent(
            new RoomWidgetUpdateChatInputContentEvent(RoomWidgetUpdateChatInputContentEvent.TEXT, chatValue),
        );
    };
    ...
    return (
        ...
        >
            <Flex fullHeight gap={hasFrank || (item.imageUrl && !imageFailed) ? 2 : 0} overflow="auto">
                ...
                <div
                    className={[
                        'notification-text overflow-y-auto flex flex-col w-full',
                        item.clickUrl && !hasFrank ? 'justify-center' : '',
                    ].join(' ')}
                >
                    {hasCommandTemplate && (
                        <div className="notification-command-template">
                            {commandTemplateContent.intro.map((text, index) => (
                                <div
                                    key={index}
                                    className={
                                        index === 0 ? 'notification-command-heading' : 'notification-command-copy'
                                    }
                                >
                                    {text}
                                </div>
                            ...
                            {commandTemplateContent.commands.map((entry, index) => (
                                ...
                                >
                                    ...
                                    {entry.description && (
                                        <span className="notification-command-description">{entry.description}</span>
                                    )}
                                </button>
                            ))}
                        </div>

...
export const WiredConditionActorIsGroupMemberView: FC<WiredConditionActorIsGroupMemberViewProps> = ({
    negative = false,
}) => {
                    ...
                    {[0, 1].map((value) => (
                        <label key={value} className="flex items-center gap-1">
                            <input
                                ...
                                name="conditionGroupQuantifier"
                                type="radio"
                                onChange={() => setQuantifier(value)}
                            />
                            <Text>
                                {LocalizeText(`wiredfurni.params.quantifier.users${negative ? '.neg' : ''}.${value}`)}
                            </Text>
                        </label>
                    ))}
                </div>

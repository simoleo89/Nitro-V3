...
const CatalogClassicViewInner: FC<{}> = () => {
    ...
    return (
        <>
            {isVisible && (
                <NitroCardView
                    classNames={['habbo-swf-window', 'habbo-swf-catalog-window', 'nitro-catalog-classic-window']}
                    isResizable={false}
                    uniqueKey="catalog"
                >
                    <NitroCardHeaderView
                    ...
                    {adminMode && (
                        <button
                            ...
                            title={
                                hasPendingChanges ? 'You have unsaved changes - click to publish' : 'Publish catalog'
                            }
                        >
                            {loading ? '...' : 'PUBLISH'}
                        </button>
                    ...
                    <NitroCardTabsView classNames={['nitro-catalog-classic-tabs-shell']} justifyContent="start">
                        {rootNode &&
                            ...
                            rootNode.children.map((child, index) => {
                                ...
                                if (
                                    !adminMode &&
                                    index === 0 &&
                                    getSwfTabLabel(child.localization).toLowerCase().includes('rari')
                                )
                                    return null;
                                    ...
                                    >
                                        <div className={`flex items-center gap-1 ${isHidden ? 'opacity-40' : ''}`}>
                                            {child.iconId > 0 && (
                                                <CatalogIconView
                                                    icon={child.iconId}
                                                    className="nitro-catalog-classic-tab-icon"
                                                />
                                            )}
                                            <span className="nitro-catalog-classic-tab-label truncate">
                                                {getSwfTabLabel(child.localization)}
                                            </span>
                                            {adminMode && isHidden && (
                                                <FaEyeSlash className="text-[8px] text-danger ml-1" />
                                            )}
                                            {adminMode && (
                                                <div
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        ...
                                                        title={
                                                            isHidden
                                                                ? LocalizeText('catalog.admin.show')
                                                                : LocalizeText('catalog.admin.hide')
                                                        }
                                                    ...
                                                    <FaTrash
                                                        ...
                                                        onClick={() => {
                                                            if (
                                                                confirm(
                                                                    LocalizeText(
                                                                        'catalog.admin.delete.category.confirm',
                                                                        ['name'],
                                                                        [child.localization],
                                                                    ),
                                                                )
                        ...
                        {isMod && (
                            <NitroCardTabsItemView
                                classNames={['nitro-catalog-classic-admin-tab']}
                                isActive={adminMode}
                                onClick={() => setAdminMode(!adminMode)}
                            >
                                <FaCog
                                    className={`text-[10px] ${adminMode ? 'animate-spin' : ''}`}
                                    style={adminMode ? { animationDuration: '3s' } : {}}
                                />
                            </NitroCardTabsItemView>
                        )}
                    </NitroCardTabsView>
                    <div className="nitro-catalog-classic-swf-header">
                        <div
                            className="nitro-catalog-classic-swf-header-bg"
                            style={
                        ...
                        <div className="nitro-catalog-classic-swf-header-copy">
                            ...
                            {currentType === CatalogType.BUILDER ? (
                                <div className="nitro-catalog-classic-swf-header-description">
                                    {LocalizeText('builder.header.status.membership')}
                                </div>
                            ) : (
                                <div
                                    className="nitro-catalog-classic-swf-header-description"
                                    dangerouslySetInnerHTML={{
                                        __html: SanitizeHtml(currentPage?.localization?.getText(0) || ''),
                                    }}
                                />
                            )}
                        </div>
                    ...
                    <NitroCardContentView classNames={['nitro-catalog-classic-content-shell']}>
                        ...
                        <div
                            className={`nitro-catalog-classic-stage ${navigationHidden ? 'is-navigation-hidden' : ''}`}
                        >
                            {!navigationHidden && (
                            ...
                            <div className="nitro-catalog-classic-layout-shell">
                                <div className="nitro-catalog-classic-layout-header-shell">
                                    ...
                                    <div className="nitro-catalog-classic-layout-hero">
                                        {!!currentPage?.localization?.getImage(0) && (
                                            <img src={currentPage.localization.getImage(0)} />
                                        )}
                                    </div>
                                </div>

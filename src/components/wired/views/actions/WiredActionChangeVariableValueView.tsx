...
import {
    CLICKED_USER_SOURCE,
    FURNI_SOURCES,
    sortWiredSourceOptions,
    USER_SOURCES,
    useAvailableUserSources,
    WiredSourceOption,
} from '../WiredSourcesSelector';
...
const serializeStringData = (
    destinationVariableToken: string,
    referenceVariableToken: string,
    referenceFurniIds: number[],
...
const resolveSourceOptions = (
    baseOptions: WiredSourceOption[],
    selectedValue: number,
    fallbackOptions: WiredSourceOption[],
) => {
...
export const WiredActionChangeVariableValueView: FC<{}> = () => {
    ...
    const resolvedDestinationVariableEntries = useMemo(() => {
        ...
        if (
            flattenWiredVariablePickerEntries(destinationVariableEntries).some(
                (entry) => entry.token === destinationVariableToken,
            )
        )
    ...
    const resolvedReferenceVariableEntries = useMemo(() => {
        ...
        if (
            flattenWiredVariablePickerEntries(referenceVariableEntries).some(
                (entry) => entry.token === referenceVariableToken,
            )
        )
    ...
    const destinationSelectionEnabled =
        isFurniTarget(destinationTargetType) && destinationFurniSource === SOURCE_SELECTED;
    const referenceSelectionEnabled =
        referenceMode === 'variable' &&
        isFurniTarget(referenceTargetType) &&
        referenceFurniSource === SOURCE_SECONDARY_SELECTED;
    ...
    const destinationSourceOptions = useMemo(() => {
        ...
        if (isFurniTarget(destinationTargetType))
            return resolveSourceOptions(orderedFurniSources, destinationSelectedSourceValue, orderedFurniSources);
        ...
        destinationSelectedSourceValue,
        destinationTargetType,
        orderedFurniSources,
        orderedUserSources,
        userSourceFallbackOptions,
    ]);
    ...
    const referenceSourceOptions = useMemo(() => {
        ...
        if (isFurniTarget(referenceTargetType))
            return resolveSourceOptions(SECONDARY_FURNI_SOURCES, referenceSelectedSourceValue, SECONDARY_FURNI_SOURCES);
    ...
    useEffect(() => {
        ...
        const nextDestinationTargetType = normalizeTargetType(
            trigger.intData.length > 0 ? trigger.intData[0] : TARGET_USER,
        );
        const nextReferenceTargetType = normalizeTargetType(
            trigger.intData.length > 4 ? trigger.intData[4] : TARGET_USER,
        );
        ...
        setReferenceMode(
            (trigger.intData.length > 2 ? trigger.intData[2] : REFERENCE_CONSTANT) === REFERENCE_VARIABLE
                ? 'variable'
                : 'constant',
        );
        ...
        setDestinationFurniSource(
            trigger.intData.length > 6
                ? trigger.intData[6]
                : nextDestinationFurniIds.length
                  ? SOURCE_SELECTED
                  : SOURCE_TRIGGER,
        );
        setReferenceUserSource(trigger.intData.length > 7 ? trigger.intData[7] : SOURCE_TRIGGER);
        setReferenceFurniSource(
            trigger.intData.length > 8
                ? trigger.intData[8]
                : nextReferenceFurniIds.length
    ...
    useEffect(
        () => syncHighlights(destinationFurniIds, referenceFurniIds),
        [destinationFurniIds, referenceFurniIds, syncHighlights],
    );
    ...
    useEffect(() => {
        ...
        const canEditSelection =
            selectionMode === 'destination' ? destinationSelectionEnabled : referenceSelectionEnabled;

        setAllowsFurni(
            canEditSelection ? WiredFurniType.STUFF_SELECTION_OPTION_BY_ID : WiredFurniType.STUFF_SELECTION_OPTION_NONE,
        );
    ...
    const save = () => {
        ...
        setStringParam(
            serializeStringData(
                ...
                referenceMode === 'variable' ? referenceVariableToken : '',
                nextReferenceFurniIds,
            ),
        );
        setIntParams([
                ...
                <div className="nitro-wired__give-var-section">
                    <WiredFurniSelectionSourceRow
                        ...
                        onChange={(value) => {
                            ...
                            if (!isGlobalTarget(destinationTargetType) && !isContextTarget(destinationTargetType))
                                setDestinationUserSource(value);
                        }}
                        ...
                        <div className="nitro-wired__give-var-section">
                            <WiredFurniSelectionSourceRow
                                ...
                                onChange={(value) => {
                                    ...
                                    if (!isGlobalTarget(referenceTargetType) && !isContextTarget(referenceTargetType))

...
const resolveLoginLocale = (value: string): LoginLocale => {
    ...
    return (
        LOGIN_LOCALES.find((locale) => normalizeLanguageCode(locale.code).split('-')[0] === base) || LOGIN_LOCALES[0]
    );
};
...
const readCachedLocale = (): LoginLocale => {
    try {
        ...
        if (typeof settings.uiTextLanguage === 'string' && settings.uiTextLanguage.length)
            return resolveLoginLocale(settings.uiTextLanguage);
    } catch {}
...
const LoginSubmitButton: FC<{ isEntering: boolean; isLocked: boolean; loginPingingServer: boolean }> = ({
    isEntering,
    isLocked,
    loginPingingServer,
}) => {
...
export const LoginView: FC<LoginViewProps> = ({ onAuthenticated, isEntering = false }) => {
    ...
    const [loginViewConfig, setLoginViewConfig] = useState<Record<string, unknown>>(() =>
        GetConfigurationValue<Record<string, unknown>>('loginview', {}),
    );
    const submitTimeRef = useRef(0);
    const preloadedLoginImagesRef = useRef<Set<string>>(new Set());

    const configuredLoginImages = useMemo<Record<string, string>>(
        () => (loginViewConfig?.['images'] as Record<string, string>) ?? {},
        [loginViewConfig],
    );
    const loginImages = useMemo<Record<string, string>>(
        () => ({ ...getDefaultLoginImages(), ...configuredLoginImages }),
        [configuredLoginImages],
    );

    const configuredLoginWidgets = useMemo<Record<string, unknown>>(
        () => (loginViewConfig?.['widgets'] as Record<string, unknown>) ?? {},
        [loginViewConfig],
    );
    ...
    const loginWidgetSlots = useMemo(() => {
        return Object.entries(configuredLoginWidgets)
            .filter(
                ([key, value]) =>
                    key.startsWith('slot.') && key.endsWith('.widget') && typeof value === 'string' && value.length > 0,
            )
            .map(([key, value]) => {
    ...
    const backgroundColor =
        loginImages['background.colour'] || GetConfigurationValue<string>('login_background.colour', '#6eadc8');
    ...
    const rightRepeat = interpolate(
        loginImages['right.repeat'] || GetConfigurationValue<string>('login_right.repeat', ''),
    );
    ...
    const loginAction = useCallback(
        async (_prev: null, formData: FormData): Promise<null> => {
            ...
            if (lockSnapshot.lockedUntil > nowTs) {
                ...
                setError(
                    t(
                        'nitro.login.error.too_many_attempts',
            ...
            try {
                ...
                const ssoTicket =
                    typeof payload.ssoTicket === 'string'
                        ? payload.ssoTicket
                        : typeof payload.sso === 'string'
                          ? payload.sso
                          : '';
                ...
                if (ok && ssoTicket) {
                    ...
                    if (rememberFlag)
                        StoreRememberLoginFromPayload(
                            payload,
                            typeof payload.username === 'string' ? payload.username : usernameInput,
                            ssoTicket,
                        );
                ...
                const message =
                    typeof payload.error === 'string'
                        ? payload.error
                        : t('nitro.login.error.invalid_credentials', 'Invalid Habbo name or password.');
                setError(message);
                resetLoginTurnstile();
            } catch {
                recordFailure();
                setError(
                    t('nitro.login.error.login_unreachable', 'Unable to reach the login service. Please try again.'),
                );
                resetLoginTurnstile();
            }
    ...
    const interpretAvailability = (
        ...
        if (ok) {
            if (
                isTrue(payload.available) ||
                isFalse(payload.exists) ||
                isFalse(payload.taken) ||
                isFalse(payload.inUse) ||
                isFalse(payload.in_use)
            )
                return { available: true };
            if (
                isFalse(payload.available) ||
                isTrue(payload.exists) ||
                isTrue(payload.taken) ||
                isTrue(payload.inUse) ||
                isTrue(payload.in_use)
            )
    ...
    const checkEmailAvailable = useCallback(
        async (email: string): Promise<{ available: boolean; error?: string }> => {
            try {
                ...
                return {
                    available: false,
                    error: result.error || t('nitro.login.error.email_taken', 'This email is already in use.'),
                };
            } catch {
                return { available: true };
            }
    ...
    const checkUsernameAvailable = useCallback(
        async (username: string): Promise<{ available: boolean; error?: string }> => {
            try {
                ...
                return {
                    available: false,
                    error: result.error || t('nitro.login.error.username_taken', 'This Habbo name is already taken.'),
                };
            } catch {
                return { available: true };
            }
        ...
        async (
            ...
            try {
                ...
                setError(
                    typeof payload.error === 'string'
                        ? payload.error
                        : t('nitro.login.error.register_failed', 'Unable to create your account.'),
                );
                onDialogReset();
            } catch {
        ...
        async (body: { email: string; turnstileToken: string }, onDialogReset: () => void) => {
            ...
            try {
                ...
                setError(
                    typeof payload.error === 'string'
                        ? payload.error
                        : t('nitro.login.error.forgot_failed', 'Unable to send a reset email right now.'),
                );
                onDialogReset();
            } catch {
    ...
    return (
        <div className="nitro-login-view" style={backgroundColor ? { background: backgroundColor } : undefined}>
            {background ? (
                <img
                    className="login-background login-layer login-layer-img"
                    src={background}
                    alt=""
                    draggable={false}
                />
            ...
            {drape ? (
                <img className="login-drape login-layer login-layer-img" src={drape} alt="" draggable={false} />
            ) : null}
            {left ? (
                <img className="login-left login-layer login-layer-img" src={left} alt="" draggable={false} />
            ) : null}
            {rightRepeat ? (
                <div className="login-right-repeat login-layer" style={{ backgroundImage: `url(${rightRepeat})` }} />
            ) : null}
            {right ? (
                <img className="login-right login-layer login-layer-img" src={right} alt="" draggable={false} />
            ) : null}
            ...
            <div className="login-stack">
                <div className="nitro-login-card login-language-card">
                            ...
                            <button
                                ...
                                style={
                                    selectedLocale.code === locale.code
                                        ? { backgroundImage: `url(${flagSelected})` }
                                        : undefined
                                }
                            >
                    ...
                    <button
                        type="button"
                        className="ok-button login-language-confirm"
                        disabled={localeApplying}
                        onClick={confirmLocaleSelection}
                    >
                        {localeApplying
                            ? t('nitro.login.language.loading', 'Loading...')
                            : t('nitro.login.language.ok', 'OK')}
                    </button>
                </div>
                    ...
                    <form className="card-body" action={submitLoginAction} autoComplete="on">
                        ...
                        {loginServerReachable === false && (
                            <div className="error-line server-offline">
                                {t(
                                    'nitro.login.server.offline.short',
                                    "The gameserver isn't running right now. Please try again in a moment.",
                                )}
                                <button
                                    type="button"
                                    className="retry-link"
                                    onClick={pingLoginServer}
                                    disabled={loginPingingServer}
                                >
                                    {loginPingingServer
                                        ? t('nitro.login.server.checking', 'Checking…')
                                        : t('nitro.login.server.retry', 'Retry')}
                                </button>
                            </div>
                        )}
                        ...
                        <div className="submit-row">
                            <LoginSubmitButton
                                ...
                                isLocked={isLocked}
                                loginPingingServer={loginPingingServer}
                            />
                        </div>
...
const RegisterDialog: FC<RegisterDialogProps> = (props) => {
    ...
    const credentialsAction = useCallback(
        async (_prev: null, _formData: FormData): Promise<null> => {
            ...
            if (password.length < 8) {
                setLocalError(
                    t('nitro.login.register.error.password_too_short', 'Your password must be at least 8 characters.'),
                );
                return null;
            }
            if (password !== confirm) {
            ...
            if (!serverOk) {
                setLocalError(
                    t('nitro.login.error.server_offline', 'The gameserver is not running. Please try again later.'),
                );
                return null;
            }
    ...
    const avatarAction = useCallback(
        async (_prev: null, _formData: FormData): Promise<null> => {
            ...
            if (!result.available) {
                setLocalError(
                    result.error || t('nitro.login.error.username_taken', 'This Habbo name is already taken.'),
                );
                return null;
            }
    ...
    useEffect(() => {
        ...
        fetch(roomTemplatesUrl, { credentials: 'include' })
            ...
            .catch(() => {
                if (cancelled) return;
                setRoomTemplates([]);
                setRoomTemplatesError(
                    t('nitro.login.register.room.error', 'Could not load room options. You can still skip this step.'),
                );
            });
        return () => {
            cancelled = true;
    ...
    return (
        <div className="nitro-login-modal">
            <div
                className={`dialog ${step === 'avatar' ? 'dialog-avatar' : ''} ${step === 'room' ? 'dialog-room' : ''}`}
            >
                <div className="nitro-login-card">
                    <div className="card-title">
                        <span>{t('nitro.login.register.title', 'Habbo Details')}</span>
                        <span
                            className="nitro-card-close-button"
                            role="button"
                        ...
                        <form className="card-body" action={submitCredentialsAction} autoComplete="on">
                            ...
                            <div className="field">
                                <label htmlFor="register-confirm">
                                    {t('nitro.login.register.confirm_password', 'Confirm password')}
                                </label>
                                <input
                                    id="register-confirm"
                                    name="confirm"
                            ...
                            <div className="step-footer">
                                ...
                                <button
                                    type="submit"
                                    className="ok-button"
                                    disabled={!credentialsValid || busy || serverOffline}
                                >
                    ...
                    {step === 'avatar' && (
                        <form className="card-body" action={submitAvatarAction} autoComplete="on">
                            ...
                            <div className="hot-looks-row">
                                <button
                                    ...
                                    title={
                                        hotLooks.length
                                            ? t(
                                                  'nitro.login.register.hotlooks.available',
                                                  '%count% looks available',
                                                  ['count'],
                                                  [String(hotLooks.length)],
                                              )
                                            : t('nitro.login.register.hotlooks.none', 'No hot looks loaded')
                                    }
                                >
                                    {t('nitro.login.register.hotlooks', 'Hot Looks')}
                                    {hotLookIndex >= 0 && hotLooks.length
                                        ? ` (${hotLookIndex + 1}/${hotLooks.length})`
                                        : ''}
                                </button>
                            </div>
                            ...
                            <div className="step-footer step-footer-split">
                                <button
                                    type="button"
                                    className="ok-button back-button"
                                ...
                                <button
                                    type="submit"
                                    className="ok-button"
                                    disabled={!username.trim() || busy || serverOffline}
                                >
                                    {isAvatarPending
                                        ? t('nitro.login.server.checking', 'Checking…')
                                        : pingingServer
                    ...
                    {step === 'room' && (
                        <form className="card-body" action={submitRoomAction} autoComplete="off">
                            <div className="register-intro">
                                {t(
                                    'nitro.login.register.intro.room',
                            ...
                            <div className="room-templates-list">
                                <label
                                    className={`room-template-option room-template-skip ${selectedTemplateId === null ? 'selected' : ''}`}
                                >
                                    <input
                                        type="radio"
                                    ...
                                    <div className="room-template-body">
                                        <div className="room-template-title">
                                            {t(
                                                'nitro.login.register.room.skip.title',
                                                "I'm okay — I'll create my own rooms",
                                            )}
                                        </div>
                                        <div className="room-template-description">
                                            {t(
                                                'nitro.login.register.room.skip.description',
                            ...
                            <div className="step-footer step-footer-split">
                                ...
                                <button type="submit" className="ok-button" disabled={busy || serverOffline}>
                                    {submitting || isRoomPending
                                        ? t('nitro.login.register.creating', 'Creating…')
                                        : t('nitro.login.register.finish', 'Finish')}
                                </button>
                            </div>
                        </form>
                    )}

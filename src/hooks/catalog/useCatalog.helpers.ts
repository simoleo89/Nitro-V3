 ...
 */
export const buildCatalogNodeTree = (
    root: NodeData,
): { rootNode: ICatalogNode; offersToNodes: Map<number, ICatalogNode[]> } => {
...
export const resolveBuilderFurniPlaceableStatus = (input: BuilderPlacementStatusInput): BuilderFurniPlaceableStatus => {
    ...
    const canUseGuildAdminFallback =
        roomSession.isGuildRoom && roomSession.controllerLevel >= RoomControllerLevel.GUILD_ADMIN && secondsLeft > 0;

    const usesSharedPlacementPool =
        !roomSession.isRoomOwner && (builderPlacementAllowedInCurrentRoom || canUseGuildAdminFallback);

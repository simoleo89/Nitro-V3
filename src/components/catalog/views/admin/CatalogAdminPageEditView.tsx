...
export const CatalogAdminPageEditView: FC = () => {
    ...
    useEffect(() => {
        ...
        setParentId(
            typeof wireParentId === 'number' && wireParentId !== -1
                ? wireParentId
                : targetNode.parent
                  ? targetNode.parent.pageId
                  : -1,
        );
    ...
    const inputClass =
        'text-[11px] border-2 border-card-grid-item-border rounded px-2 py-1 bg-white focus:outline-none focus:border-primary transition-colors';
    ...
    const handleDelete = async () => {
        ...
        if (!confirm(LocalizeText('catalog.admin.delete.page.confirm', ['name'], [editingPageDetails?.caption ?? ''])))
            return;
            ...
            <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-bold text-primary uppercase tracking-wide">
                    {isRoot
                        ...
                        : `${LocalizeText('catalog.admin.edit')} ${editingPageDetails?.caption ?? ''}`}
                </span>
            ...
            <div className="grid grid-cols-3 gap-1.5">
                ...
                <div className="flex flex-col gap-0.5 col-span-2">
                    ...
                    <input
                        className={inputClass}
                        value={captionSave}
                        onChange={(e) => setCaptionSave(e.target.value)}
                    />
                </div>
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">
                ...
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">
                        {LocalizeText('catalog.admin.order')}
                    </label>
                    <input
                        className={inputClass}
                        min={0}
                ...
                <div className="flex flex-col gap-0.5 col-span-3">
                    ...
                    <textarea
                        ...
                        value={pageText1}
                        onChange={(e) => setPageText1(e.target.value)}
                    />
                </div>
            </div>

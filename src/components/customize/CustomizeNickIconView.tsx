...
export const CustomizeNickIconView: FC = () => {
    ...
    const combinedPrefixes = useMemo(() => {
        ...
        for (const prefix of prefixItems) {
            if (prefix.catalogPrefixId && prefix.catalogPrefixId > 0)
                ownedByCatalogId.set(prefix.catalogPrefixId, prefix);
        }
    ...
    const selectedFontOption = useMemo(
        () => PRESET_PREFIX_FONTS.find((font) => font.id === customPrefixFont) || PRESET_PREFIX_FONTS[0],
        [customPrefixFont],
    );
    ...
    const customPrefixTotalPoints = useMemo(
        () =>
            customPrefixPricePoints +
            (customPrefixFont && customPrefixFontPointsType === customPrefixPointsType
                ? customPrefixFontPricePoints
                : 0),
        [
            customPrefixFont,
            customPrefixFontPointsType,
            customPrefixFontPricePoints,
            customPrefixPointsType,
            customPrefixPricePoints,
        ],
    );
    ...
    const handleCustomPrefixPurchase = () => {
        ...
        setIsLoading(true);
        SendMessageComposer(
            new PurchasePrefixComposer(
                customPrefixText.trim(),
                customPrefixColor,
                customPrefixIcon,
                customPrefixEffect,
                customPrefixFont,
    ...
    return (
        <NitroCardView
            className="customize-nick-icon-window w-[680px] max-w-[95vw]"
            theme="primary-slim"
            uniqueKey="customize-nick-icons"
        >
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col items-center gap-1 text-center text-[11px]">
                                            <span>
                                                {item.owned ? (item.active ? 'Owned - Active' : 'Owned') : 'Locked'}
                                            </span>
                                            <span className="max-w-[140px] truncate">
                                                {item.displayName || `Icon #${item.iconKey}`}
                                            </span>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {combinedPrefixes.map((item) => (
                                        ...
                                        >
                                            ...
                                            <Button
                                                disabled={isLoading}
                                                onClick={() => handleCombinedPrefixAction(item)}
                                            >
                                                {!item.owned && 'Buy'}
                                                {item.owned && !item.active && 'Activate'}
                                                {item.owned && item.active && 'Deactivate'}
                            ...
                            <div className="rounded border border-black/10 bg-black/5 p-3">
                                ...
                                <div className="mt-2 flex flex-col gap-2">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        {!!customPrefixIcon && (
                                            <Button onClick={() => setCustomPrefixIcon('')}>Clear</Button>
                                        )}
                                    </div>
                                    <div className="rounded border border-black/10 bg-white p-2">
                                        <div className="mb-2 text-[11px] leading-4 text-black/70">
                                            Safe colors only, chosen to stay readable on both light and dark
                                            backgrounds.
                                        </div>
                                    ...
                                    <div
                                        className="rounded border border-black/10 bg-[#cfe8fb] px-3 py-2 text-[#1f2937]"
                                        style={customPrefixPreviewStyle}
                                    >
                                        <UserIdentityView
                                            displayOrder={displayOrder}
                                            nickIcon={activeIcon?.iconKey || ''}
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 text-[12px]">
                                            {customPrefixTotalCredits > 0 && (
                                                <span>{customPrefixTotalCredits} credits</span>
                                            )}
                                            ...
                                            {!!customPrefixFont &&
                                                customPrefixFontPointsType !== customPrefixPointsType &&
                                                customPrefixFontPricePoints > 0 && (
                                                    <span className="inline-flex items-center gap-1">
                                        ...
                                        <Button
                                            disabled={!customPrefixIsValid || isLoading}
                                            onClick={handleCustomPrefixPurchase}
                                        >
                                            Buy custom prefix
                                        </Button>
                                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="rounded border border-black/10 bg-black/5 p-3">
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-2">
                                {Object.entries(ORDER_LABELS).map(([key, label]) => (
                                    <Button
                                        key={key}
                                        disabled={isLoading && displayOrder === key}
                                        onClick={() => handleDisplayOrderChange(key)}
                                    >
                                        {displayOrder === key ? '* ' : ''}
                                        {label}
                                    </Button>
                        ...
                        <div className="rounded border border-black/10 bg-black/5 p-3">
                            ...
                            <div className="text-[11px] leading-4 text-black/70">
                                Use this tab to control how your icon, prefix and username are ordered in bubbles,
                                profile and infostand.
                            </div>
                        </div>
                    </div>
                )}

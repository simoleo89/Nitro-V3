            ...
            {Array.from(stuffDatas.entries()).map(([objectId, stuffData], index) => {
                return (
                    <DraggableWindow key={index} uniqueKey={`high-score-${objectId}`}>
                        <Column
                            className="nitro-widget-high-score nitro-context-menu bg-[#1e1f23] p-2 w-[280px] max-w-[280px] h-[320px]"
                            gap={0}
                        >
                            <ContextMenuHeaderView classNames={['drag-handler cursor-move']}>
                                {LocalizeText(
                                    ...
                                    [
                                        LocalizeText(
                                            `high.score.display.scoretype.${getScoreType(stuffData.scoreType)}`,
                                        ),
                                        LocalizeText(
                                            `high.score.display.cleartype.${getClearType(stuffData.clearType)}`,
                                        ),
                                    ],
                                )}

...
const useMessengerState = () => {
    ...
    const visibleThreads = useMemo(
        () => messageThreads.filter((thread) => hiddenThreadIds.indexOf(thread.threadId) === -1),
        [messageThreads, hiddenThreadIds],
    );
    ...
    const getMessageThread = (userId: number) => {
        ...
        if (!thread) {
            ...
            thread.addMessage(
                ...
                0,
                null,
                MessengerThreadChat.SECURITY_NOTIFICATION,
            );
    ...
    useMessageEvent<NewConsoleMessageEvent>(NewConsoleMessageEvent, (event) => {
        ...
        if (thread.threadId === activeThreadId && parser.senderId > 0)
            SendMessageComposer(new MarkConsoleReadComposer(parser.senderId));
    });
    ...
    useMessageEvent<ConsoleReadReceiptEvent>(ConsoleReadReceiptEvent, (event) => {
        ...
        setMessageThreads((prevValue) => {
            const index = prevValue.findIndex(
                (thread) => thread.participant && thread.participant.id === parser.readerId,
            );

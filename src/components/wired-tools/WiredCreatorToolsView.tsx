...
export const WiredCreatorToolsView: FC<{}> = () => {
    ...
    const shouldPauseVariableSnapshotRefresh =
        !!editingVariable || !!editingManagedHolderVariableId || isInspectionGiveOpen || isManagedGiveOpen;
    ...
    const getTimeZoneOffsetMinutes = (epochMs: number, timeZone: string): number => {
        try {
            ...
            const timeZonePart =
                formatter.formatToParts(new Date(epochMs)).find((part) => part.type === 'timeZoneName')?.value ?? 'GMT';
    ...
    const getVariableAvailabilityLabel = (
        value: number,
        targetType: 'user' | 'furni' | 'global' | 'context',
    ): string => {
        ...
        if (value === 11) {
            ...
            return localizedValue && localizedValue !== 'wiredfurni.params.variables.availability.11'
                ? localizedValue
                : 'Permanent, shared between rooms';
        }

        if (value === 10) return 'Permanent';

        return targetType === 'furni' || targetType === 'global'
            ? 'While the room is active'
            : 'While the user is in the room';
    };
    ...
    const refreshSelectedUser = useCallback(
        (roomIndex: number = selectedUser?.roomIndex) => {
            ...
            const roomObject = GetRoomEngine().getRoomObject(roomSession.roomId, roomIndex, RoomObjectCategory.UNIT);
            const gender = String(
                userData.sex || roomObject?.model.getValue<string>(RoomObjectVariable.GENDER) || 'U',
            ).toUpperCase();
    ...
    useObjectSelectedEvent((event) => {
        ...
        if (
            ...
            (event.category === RoomObjectCategory.FLOOR || event.category === RoomObjectCategory.WALL)
        ) {
    ...
    useEffect(() => {
        if (!isVisible || !roomSession?.roomId || !roomSettings.canInspect || shouldPauseVariableSnapshotRefresh)
            return;
        ...
        requestUserVariables,
        shouldPauseVariableSnapshotRefresh,
    ]);
    ...
    const monitorHistoryTypeOptions = useMemo(() => {
        return [
            'ALL',
            ...Array.from(new Set([...MONITOR_LOG_ORDER, ...monitorSnapshot.history.map((entry) => entry.type)])),
        ];
    ...
    const monitorStats = useMemo<MonitorStat[]>(() => {
        ...
        return [
            ...
            {
                label: 'Recursion',
                value: `${monitorSnapshot.recursionDepthCurrent}/${Math.max(0, monitorSnapshot.recursionDepthLimit)}`,
            },
    ...
    const monitorLogs = useMemo<MonitorLog[]>(() => {
        return MONITOR_LOG_ORDER.map((type) => {
            ...
            return {
                ...
                latest:
                    amount > 0
                        ? formatMonitorLatestOccurrence(Number(log?.latestOccurrenceSeconds ?? 0), globalClock)
                        : '/',
    ...
    const currentWallLocationString = useMemo(() => {
        if (!roomSession || !selectedFurni || selectedFurni.category !== RoomObjectCategory.WALL || !selectedRoomObject)
            return null;
    ...
    const roomCustomVariableDefinitions = useMemo(() => {
        return [...roomVariableDefinitions].sort(
            (left, right) =>
                left.name.localeCompare(right.name, undefined, { sensitivity: 'base' }) || left.itemId - right.itemId,
        );
    }, [roomVariableDefinitions]);
    ...
    const userVariables = useMemo(() => {
        ...
        const currentControllerLevel =
            selectedUser.kind === 'user'
                ? selectedUser.roomIndex === roomSession?.ownRoomIndex
                    ...
                    : Number(
                          selectedUserRoomObject?.model.getValue<number>(RoomObjectVariable.FIGURE_FLAT_CONTROL) ??
                              selectedUser.level ??
                              0,
                      )
        ...
        const isSelectedUserGroupAdmin =
            ...
            currentControllerLevel >= RoomControllerLevel.GUILD_ADMIN;
        const isSelectedUserMuted =
            Number(selectedUserRoomObject?.model.getValue<number>(RoomObjectVariable.FIGURE_IS_MUTED) ?? 0) > 0;
        ...
        const handItemValue = Number(
            selectedUserRoomObject?.model.getValue<number>(RoomObjectVariable.FIGURE_CARRY_OBJECT) ?? 0,
        );
        const expressionValue = Number(
            selectedUserRoomObject?.model.getValue<number>(RoomObjectVariable.FIGURE_EXPRESSION) ?? 0,
        );
        const effectValue = Number(
            selectedUserRoomObject?.model.getValue<number>(RoomObjectVariable.FIGURE_EFFECT) ?? 0,
        );
        const petOwnerId =
            selectedUser.kind === 'pet'
                ? Number(roomSession?.userDataManager.getPetData(selectedUser.userId)?.ownerId ?? 0)
                : 0;
        const identityKey =
            selectedUser.kind === 'pet'
                ? '@pet_id'
        ...
        if (effectValue > 0)
            dynamicUserActions.push({
                key: '@effect_id',
                value: `${effectValue} (${getEffectDisplayName(effectValue)})`,
            });
        if (teamData) dynamicUserActions.push({ key: '@team_score', value: String(teamData.score) });
        if (teamData)
            dynamicUserActions.push({
                key: '@team_color',
                value: `${teamData.colorId} (${getTeamColorDisplayName(teamData.colorId)})`,
            });
        if (teamData)
            dynamicUserActions.push({
                key: '@team_type',
                value: `${teamData.typeId} (${getTeamTypeDisplayName(teamData.typeId)})`,
            });
        if (signValue >= 0)
            dynamicUserActions.push({ key: '@sign', value: `${signValue} (${getSignDisplayName(signValue)})` });
        if (danceValue > 0)
            dynamicUserActions.push({ key: '@dance', value: `${danceValue} (${getDanceDisplayName(danceValue)})` });
        if (expressionValue === AvatarExpressionEnum.IDLE.ordinal)
            dynamicUserActions.push({ key: '@is_idle', value: '' });
        if (handItemValue > 0)
            dynamicUserActions.push({
                key: '@handitem_id',
                value: `${handItemValue} (${getHandItemDisplayName(handItemValue)})`,
            });
        ...
        return [
            ...
            {
                key: '@type',
                value: `${selectedUser.kind === 'user' ? 1 : selectedUser.kind === 'pet' ? 2 : 4} (${selectedUser.kind})`,
            },
            {
                key: '@gender',
                value: `${selectedUser.gender === 'F' ? 1 : selectedUser.gender === 'M' ? 0 : -1} (${selectedUser.gender || 'U'})`,
            },
            ...
            ...(selectedUser.roomEntryMethod && selectedUser.roomEntryMethod !== 'unknown'
                ? [
                      {
                          key: '@room_entry.method',
                          value: `${getRoomEntryMethodNumericValue(selectedUser.roomEntryMethod)} (${selectedUser.roomEntryMethod})`,
                      },
                  ]
                : []),
    ...
    const globalVariables = useMemo(() => {
        ...
        const hotelCurrentTimeMs =
            hotelTimeSnapshotMs > 0 && hotelTimeSyncMs > 0
    ...
    const displayedVariables =
        inspectionType === 'user' ? userVariables : inspectionType === 'global' ? globalVariables : furniVariables;
    ...
    const selectedInspectionGiveDefinition = useMemo(() => {
        ...
        return (
            availableInspectionDefinitions.find((definition) => definition.itemId === inspectionGiveVariableItemId) ??
            availableInspectionDefinitions[0]
        );
    ...
    const canRemoveInspectionVariable =
        ...
        !selectedInspectionCustomDefinition.isReadOnly;
    ...
    const selectedVariableDefinition = useMemo(() => {
        ...
        return (
            variablePickerDefinitions.find((variable) => variable.key === selectedKey) ??
            variablePickerDefinitions[0] ??
            null
        );
    ...
    const selectedVariableProperties = useMemo(() => {
        ...
        return [
            ...
            {
                key: 'Can write to',
                value: selectedVariableDefinition.canWriteTo && roomSettings.canModify ? 'Yes' : 'No',
            },
    ...
    const variableManageEntries = useMemo((): VariableManageEntry[] => {
        if (!selectedVariableDefinition?.itemId || selectedVariableDefinition.type !== 'Custom' || !roomSession)
            return [];
        ...
        if (variablesType === 'user') {
            ...
            for (const [userIdString, assignments] of Object.entries(userVariableAssignments)) {
                ...
                const assignment = assignments.find(
                    (entry) => entry.variableItemId === selectedVariableDefinition.itemId,
                );
        ...
        if (variablesType === 'furni') {
            ...
            for (const [furniIdString, assignments] of Object.entries(furniVariableAssignments)) {
                ...
                const floorObject = GetRoomEngine().getRoomObject(
                    roomSession.roomId,
                    furniId,
                    RoomObjectCategory.FLOOR,
                );
                const wallObject = floorObject
                    ? null
                    : GetRoomEngine().getRoomObject(roomSession.roomId, furniId, RoomObjectCategory.WALL);
    ...
    const variableHighlightTargets = useMemo((): VariableHighlightTarget[] => {
        if (!isVariableHighlightActive || !canVariableHighlight || !roomSession || !selectedVariableDefinition?.itemId)
            return [];
        ...
        if (variablesType === 'user') {
            ...
            for (const [userIdString, assignments] of Object.entries(userVariableAssignments)) {
                ...
                targets.push({
                    ...
                    hasValue:
                        !!assignment.hasValue &&
                        !!selectedVariableDefinition.hasValue &&
    ...
    const selectedManagedHolderVariableEntry = useMemo(() => {
        ...
        return (
            managedHolderVariableEntries.find((entry) => entry.variableItemId === selectedManagedHolderVariableId) ??
            managedHolderVariableEntries[0]
        );
    ...
    const selectedManagedGiveDefinition = useMemo(() => {
        ...
        return (
            availableManagedHolderDefinitions.find((definition) => definition.itemId === managedGiveVariableItemId) ??
            availableManagedHolderDefinitions[0]
        );
    ...
    const managedHolderFurniCategory = useMemo(() => {
        ...
        return selectedManagedVariableEntry.categoryLabel === 'Wall furni'
    ...
    const canGiveManagedHolderVariable =
        ...
        variablesType !== 'global' &&
        variablesType !== 'context' &&
        !!availableManagedHolderDefinitions.length;
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (managedHolderVariableEntries.some((entry) => entry.variableItemId === selectedManagedHolderVariableId))
            return;
        ...
        setSelectedManagedHolderVariableId(
            preferredEntry?.variableItemId ?? managedHolderVariableEntries[0].variableItemId,
        );
    ...
    useEffect(() => {
        ...
        if (availableManagedHolderDefinitions.some((definition) => definition.itemId === managedGiveVariableItemId))
            return;
    ...
    const variableManagePageCount = Math.max(
        1,
        Math.ceil(filteredVariableManageEntries.length / variableManagePageSize),
    );
    ...
    const variableManageCanOpen =
        !!selectedVariableDefinition && selectedVariableDefinition.type === 'Custom' && variablesType !== 'context';
    const commitManagedHolderValueEdit = useCallback(() => {
        ...
        switch (variablesType) {
            case 'user':
                updateUserVariableValue(
                    selectedManagedVariableEntry.entityId,
                    selectedManagedHolderVariableEntry.variableItemId,
                    nextValue,
                );
                break;
            case 'furni':
    ...
    const giveManagedHolderVariable = useCallback(() => {
        ...
        if (variablesType === 'user')
            assignUserVariable(selectedManagedVariableEntry.entityId, selectedManagedGiveDefinition.itemId, nextValue);
        else
            assignFurniVariable(selectedManagedVariableEntry.entityId, selectedManagedGiveDefinition.itemId, nextValue);
    ...
    const commitVariableEdit = () => {
        ...
        if (inspectionType === 'user' && selectedUser) {
            ...
            if (
                ...
                nextDirection === currentLiveState.direction
            ) {
                cancelVariableEdit();
                return;
            }

            const selectedRoomObject = GetRoomEngine().getRoomObject(
                roomSession.roomId,
                selectedUser.roomIndex,
                RoomObjectCategory.UNIT,
            );
            const currentLocation =
                selectedRoomObject?.getLocation() ??
                new Vector3d(currentLiveState.positionX, currentLiveState.positionY, currentLiveState.altitude / 100);
        ...
        const currentLiveState =
            selectedFurniLiveState ?? getFurniLiveState(selectedFurni.objectId, selectedFurni.category);
        ...
        if (editingVariable === '@state') {
            ...
            if (selectedFurni.category === RoomObjectCategory.WALL)
                SendMessageComposer(new FurnitureWallMultiStateComposer(selectedFurni.objectId, nextState));
        ...
        if (editingVariable === '@wallitem_offset') {
            ...
            const currentExtra =
                selectedFurni.info?.stuffData?.getLegacyString?.() ?? selectedFurni.info?.extraParam ?? '0';
        ...
        SendMessageComposer(
            new UpdateFurniturePositionComposer(
                selectedFurni.objectId,
                nextX,
                nextY,
                Math.round(nextZ * 10000),
                nextRotation,
            ),
        );
    ...
    useEffect(() => {
        ...
        if (availableInspectionDefinitions.some((definition) => definition.itemId === inspectionGiveVariableItemId))
            return;
    ...
    return (
        <>
            ...
            >
                <NitroCardHeaderView
                    headerText="Wired Creator Tools (:wired)"
                ...
                <NitroCardContentView className="text-black bg-[#e9e6d9]" gap={3}>
                    ...
                    {activeTab === 'inspection' && (
                        <WiredInspectionTabView
                            ...
                            onSelectInspectionVariable={(variable) => {
                                setSelectedInspectionVariableKeys((prev) => ({
                                    ...prev,
                                    [inspectionType]: variable.key,
                                }));
                    ...
                    {activeTab === 'variables' && (
                        <WiredVariablesTabView
                            ...
                            onPickVariable={(key) =>
                                setSelectedVariableKeys((prev) => ({ ...prev, [variablesType]: key }))
                            }
            ...
            {isMonitorHistoryOpen && (
                ...
                >
                    <NitroCardHeaderView
                        headerText="Wired Monitor Logs"
                        onCloseClick={() => setIsMonitorHistoryOpen(false)}
                    />
            ...
            {isMonitorInfoOpen && (
                ...
                >
                    <NitroCardHeaderView
                        headerText="Wired Monitor Information"
                        onCloseClick={() => setIsMonitorInfoOpen(false)}
                    />
            ...
            {isVariableManageOpen && !!selectedVariableDefinition && (
                ...
                >
                    <NitroCardHeaderView
                        headerText="Variable Management"
                        onCloseClick={() => setIsVariableManageOpen(false)}
                    />
                    <NitroCardContentView className="text-black bg-[#f4efe3] p-3 flex flex-col gap-3" overflow="hidden">
                            ...
                            <table className="w-full text-[12px]">
                                <thead className="bg-[#efede5] sticky top-0">
                                    <tr>
                                        ...
                                        {!!selectedVariableDefinition.hasCreationTime && (
                                            <th className="text-left px-2 py-1">Creation time</th>
                                        )}
                                        {!!selectedVariableDefinition.hasUpdateTime && (
                                            <th className="text-left px-2 py-1">Last update time</th>
                                        )}
                                ...
                                <tbody>
                                    {!pagedVariableManageEntries.length && (
                                        <tr>
                                            <td
                                                className="px-2 py-3 text-center text-[#8b8678]"
                                                colSpan={selectedVariableDefinition.hasCreationTime ? 6 : 5}
                                            >
                                    ...
                                    {pagedVariableManageEntries.map((entry, index) => (
                                        <tr
                                            ...
                                            className={index % 2 === 0 ? 'bg-white' : 'bg-[#f8f6f0]'}
                                        >
                                            <td className="px-2 py-1">{entry.categoryLabel}</td>
                                            <td className="px-2 py-1 text-[#1b57b2] underline underline-offset-2">
                                                {entry.entityName}
                                            </td>
                                            {!!selectedVariableDefinition.hasCreationTime && (
                                                <td className="px-2 py-1">
                                                    {formatVariableTimestamp(entry.createdAt)}
                                                </td>
                                            )}
                                            {!!selectedVariableDefinition.hasUpdateTime && (
                                                <td className="px-2 py-1">
                                                    {formatVariableTimestamp(entry.updatedAt)}
                                                </td>
                                            )}
                        ...
                        <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                        setVariableManagePage((value) => Math.min(variableManagePageCount, value + 1))
                                    }
                                >
                                    &rsaquo;
                                </button>
            ...
            {!!selectedManagedVariableEntry && !!selectedVariableDefinition && (
                ...
                >
                    <NitroCardHeaderView
                        headerText={managedHolderPanelTitle}
                        onCloseClick={() => setSelectedManagedVariableEntry(null)}
                    />
                    <NitroCardContentView
                        className="text-black bg-[#f4efe3] p-3 flex flex-col gap-3 relative"
                        overflow="hidden"
                    >
                        <div className="rounded border border-[#c8c2b2] bg-white p-3 flex items-center justify-between gap-3">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-4">
                                <div className="w-[140px] h-[110px] rounded border border-[#d8d2c3] bg-[#dedede] flex items-center justify-center overflow-hidden">
                                    ...
                                    {variablesType === 'user' && managedHolderUserData && (
                                        <>
                                            {managedHolderUserData.type === RoomObjectType.PET ? (
                                                <LayoutPetImageView
                                                    direction={2}
                                                    figure={managedHolderUserData.figure}
                                                />
                                            ) : (
                                                <LayoutAvatarImageView
                                                    direction={2}
                                    ...
                                    {variablesType === 'global' && (
                                        <img
                                            alt="Global placeholder"
                                            className="max-w-full max-h-full object-contain p-3"
                                            src={wiredGlobalPlaceholderImage}
                                        />
                                    )}
                                </div>
                                                ...
                                                >
                                                    <td className="px-2 py-1">
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="text-[10px] text-[#8a8476]">
                                                                {entry.availability}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className="px-2 py-1">
                                                        ...
                                                        {!!entry.hasValue && !isEditing && (
                                                            <button
                                                                ...
                                                                onClick={(event) => {
                                                                    ...
                                                                    if (!roomSettings.canModify || entry.isReadOnly)
                                                                        return;

                                                                    setSelectedManagedHolderVariableId(
                                                                        entry.variableItemId,
                                                                    );
                                                                    setEditingManagedHolderVariableId(
                                                                        entry.variableItemId,
                                                                    );
                                                                    setEditingManagedHolderValue(
                                                        ...
                                                        {!!entry.hasValue && isEditing && (
                                                            <input
                                                                ...
                                                                onChange={(event) =>
                                                                    setEditingManagedHolderValue(event.target.value)
                                                                }
                        ...
                        <div className="relative flex items-center justify-between gap-3 pt-1">
                            {isManagedGiveOpen && (
                                <div className="absolute right-0 bottom-full mb-2 w-[210px] rounded border border-[#8d887a] bg-[#efede5] p-3 shadow-[0_2px_8px_rgba(0,0,0,.25)] z-10 flex flex-col gap-2">
                                    ...
                                    >
                                        {!availableManagedHolderDefinitions.length && (
                                            <option value={0}>No variables available</option>
                                        )}
                                    ...
                                    <Button
                                        ...
                                        variant="secondary"
                                        onClick={() => giveManagedHolderVariable()}
                                    >
                                        Create
                                    </Button>
                                </div>
                            ...
                            <Button
                                disabled={!canRemoveManagedHolderVariable}
                                variant="secondary"
                                onClick={() => removeManagedHolderVariable()}
                            >
                                Remove variable
                            </Button>
                            <Button

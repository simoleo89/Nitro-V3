...
export const GroupForumSettingsView: FC<GroupForumSettingsViewProps> = (props) => {
    ...
    const [postMessagePermission, setPostMessagePermission] = useState<number>(
        forumData?.postMessagePermissions ?? PERMISSION_MEMBERS,
    );
    const [postThreadPermission, setPostThreadPermission] = useState<number>(
        forumData?.postThreadPermissions ?? PERMISSION_MEMBERS,
    );
    const [moderatePermission, setModeratePermission] = useState<number>(
        forumData?.moderatePermissions ?? PERMISSION_ADMINS,
    );

    const saveSettings = useCallback(() => {
        SendMessageComposer(
            new UpdateForumSettingsMessageComposer(
                effectiveGroupId,
                readPermission,
            ...
            <Column gap={2}>
                <Column gap={1}>
                    ...
                    <select
                        ...
                        onChange={(e) => setReadPermission(parseInt(e.target.value))}
                    >
                ...
                <Column gap={1}>
                    ...
                    <select
                        className="form-select form-select-sm"
                        value={moderatePermission}
                        onChange={(e) => setModeratePermission(parseInt(e.target.value))}
                    >
                        {[
                            {
                                value: PERMISSION_ADMINS,
                                label: LocalizeText('groupforum.permissions.option_group_admins'),
                            },

...
export const HcCenterView: FC = (props) => {
        ...
        switch (clubStatus) {
            case ClubStatus.ACTIVE:
                return LocalizeText(
                    ...
                    [
                        getClubText(),
                        kickbackData?.firstSubscriptionDate,
                        FriendlyTime.shortFormat(kickbackData?.currentHcStreak * 86400),
                    ],
                );
    ...
    const getHcPaydayAmount = () =>
        LocalizeText(
            'hccenter.special.sum',
            ['credits'],
            [(kickbackData?.creditRewardForStreakBonus + kickbackData?.creditRewardForMonthlySpent).toString()],
        );
        ...
        <>
            ...
            <div>
                {LocalizeText(
                    ...
                    ['credits'],
                    [kickbackData?.totalCreditsSpent.toString()],
                )}
            </div>
            <div>
                {LocalizeText(
                    ...
                    ['percent'],
                    [(kickbackData?.kickbackPercentage * 100).toString()],
                )}
            </div>
            <div>
                {LocalizeText(
                    'hccenter.breakdown.streakbonus',
                    ['credits'],
                    [kickbackData?.creditRewardForStreakBonus.toString()],
                )}
            </div>
            ...
            <div>
                {LocalizeText(
                    ...
                    [
                        getHcPaydayAmount(),
                        (
                            ((kickbackData?.kickbackPercentage * kickbackData?.totalCreditsSpent +
                                kickbackData?.creditRewardForStreakBonus) *
                                100) /
                            100
                        ).toString(),
                    ],
    ...
    return (
        <NitroCardView className="w-[430px] resize-none" theme="primary-slim">
            ...
            <Flex className="bg-muted/50 p-3" position="relative">
                <Column gap={2}>
                    <div
                        className="w-[213px] h-[37px] bg-contain bg-no-repeat"
                        style={{ backgroundImage: `url(${hcLogo})` }}
                    />
                    <Button
                        variant="success"
                        onClick={(event) =>
                            CreateLinkEvent('catalog/open/' + GetConfigurationValue('catalog.links')['hc.buy_hc'])
                        }
                    >
                    ...
                    <Flex className="rounded overflow-hidden border border-card-grid-item-border">
                        <Column className="bg-primary p-3 flex-1 text-white" gap={1}>
                            ...
                            <div className="mt-auto">
                                <span
                                    ...
                                    onClick={() =>
                                        CreateLinkEvent(
                                            'habbopages/' + GetConfigurationValue('hc.center')['payday.habbopage'],
                                        )
                                    }
                                >
                                    {LocalizeText('hccenter.special.infolink')}
                        ...
                        >
                            ...
                            <Flex gap={1} alignItems="center">
                                <div
                                    className="w-5 h-5 shrink-0 bg-contain bg-no-repeat bg-center"
                ...
                {GetConfigurationValue('hc.center')['gift.info'] && (
                    <Flex className="rounded bg-success/90 p-3" alignItems="center" gap={2}>
                            ...
                            <Text
                                ...
                                dangerouslySetInnerHTML={{
                                    __html:
                                        unclaimedGifts > 0
                                            ? LocalizeText(
                                                  'hccenter.unclaimedgifts',
                                                  ['unclaimedgifts'],
                                                  [unclaimedGifts.toString()],
                                              )
                        ...
                        <Button
                            ...
                            className="shrink-0"
                            onClick={() =>
                                CreateLinkEvent('catalog/open/' + GetConfigurationValue('catalog.links')['hc.hc_gifts'])
                            }
                        >
                            {LocalizeText(
                                clubStatus === ClubStatus.ACTIVE
                                    ? 'hccenter.btn.gifts.redeem'
                                    : 'hccenter.btn.gifts.view',
                            )}
                        </Button>
                    </Flex>
                )}
                {GetConfigurationValue('hc.center')['benefits.info'] && (
                    ...
                    >
                        ...
                        <Text
                            ...
                            dangerouslySetInnerHTML={{ __html: LocalizeText('hccenter.general.info') }}
                        />
                        <span
                            className="text-primary text-sm cursor-pointer hover:underline mt-1"
                            onClick={() =>
                                CreateLinkEvent(
                                    'habbopages/' + GetConfigurationValue('hc.center')['benefits.habbopage'],
                                )
                            }
                        >

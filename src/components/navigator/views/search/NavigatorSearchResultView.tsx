...
export const NavigatorSearchResultView: FC<NavigatorSearchResultViewProps> = (props) => {
    ...
    const toggleDisplayMode = () => {
        setDisplayMode((prevValue) => {
            if (prevValue === NavigatorSearchResultViewDisplayMode.LIST)
                return NavigatorSearchResultViewDisplayMode.THUMBNAILS;
    ...
    const showMore = () => {
        ...
        else if (searchResult.action == 2 && topLevelContext)
            SendMessageComposer(new NavigatorSearchComposer(topLevelContext.code, ''));
    };
            ...
            <Flex fullWidth alignItems="center" className="px-2 py-1" justifyContent="between">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {searchResult.action > 0 && searchResult.action === 1 && (
                        <FaWindowMaximize
                            className="text-secondary fa-icon cursor-pointer"
                    ...
                    {searchResult.action > 0 && searchResult.action !== 1 && (
                        <FaWindowRestore
                            className="text-secondary fa-icon cursor-pointer"
                            title={LocalizeText('navigator.back')}
                            onClick={showMore}
                        />
                    )}
                    <LayoutSearchSavesView
                        ...
                        onClick={() =>
                            SendMessageComposer(new NavigatorSearchSaveComposer(getResultTitle(), searchResult.data))
                        }
                    />
                </div>
            </Flex>
                ...
                <>
                    ...
                    {searchResult.rooms.length === 0 && (
                        <Text className="px-3 py-2 text-sm" variant="muted">
                            {LocalizeText(
                                searchResult.code === 'myworld_view'
                                    ? 'navigator.roomsettings.moderation.none'
                                    : 'navigator.search.returned.no.results',
                            )}
                        </Text>
                    )}
                </>

...
const useFurnitureDimmerWidgetState = () => {
    ...
    const applyChanges = () => {
        ...
        if (
            !preset ||
            (selectedEffectId === preset.type && selectedColor === preset.color && selectedBrightness === preset.light)
        )
            return;
        ...
        setPresets((prevValue) => {
            ...
            newValue[selectedPresetIndex] = new DimmerFurnitureWidgetPresetItem(
                preset.id,
                selectedEffectId,
                selectedColor,
                selectedBrightness,
    ...
    useNitroEvent<RoomSessionDimmerPresetsEvent>(RoomSessionDimmerPresetsEvent.ROOM_DIMMER_PRESETS, (event) => {
        ...
        while (i < event.presetCount) {
            ...
            if (preset)
                presets.push(
                    new DimmerFurnitureWidgetPresetItem(preset.id, preset.type, preset.color, preset.brightness),
                );

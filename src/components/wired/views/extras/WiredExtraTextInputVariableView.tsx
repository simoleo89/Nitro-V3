...
export const WiredExtraTextInputVariableView: FC = () => {
    ...
    const targetDefinitions = useMemo(
        () => contextVariableDefinitions.filter((definition) => !!definition.hasValue),
        [contextVariableDefinitions],
    );
    const variableEntries = useMemo(
        () =>
            buildWiredVariablePickerEntries('context', 'change-reference', targetDefinitions).filter(
    ...
    const previewHtml = useMemo(
        () => LocalizeText('wiredfurni.params.texts.placeholder_preview', ['placeholder'], [escapeHtml(previewToken)]),
        [previewToken],
    );
        ...
        >
            <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    />
                    {!targetDefinitions.length && (
                        <Text small>{'No wf_var_context variables with value found in this room.'}</Text>
                    )}
                </div>

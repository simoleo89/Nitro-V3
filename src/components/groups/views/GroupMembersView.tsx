...
export const GroupMembersView: FC<{}> = (props) => {
    ...
    const toggleAdmin = (member: GroupMemberParser) => {
        ...
        if (member.rank !== GroupRank.ADMIN)
            SendMessageComposer(new GroupAdminGiveComposer(membersData.groupId, member.id));
        ...
        <NitroCardView className="w-[400px] max-h-[380px]        " theme="primary-slim">
            <NitroCardHeaderView
                headerText={LocalizeText(
                    'group.members.title',
                    ['groupName'],
                    [membersData ? membersData.groupTitle : ''],
                )}
            ...
            <NitroCardContentView overflow="hidden">
                <div className="flex gap-2">
                    ...
                    <Column fullWidth gap={1}>
                        ...
                        <select
                            ...
                            value={levelId}
                            onChange={(event) => setLevelId(parseInt(event.target.value))}
                        >
                    ...
                    {membersData.result.map((member, index) => {
                        return (
                            <Flex
                                key={index}
                                alignItems="center"
                                className="p-2 bg-white rounded h-[50px] max-h-[50px]"
                                gap={2}
                                overflow="hidden"
                            >
                                <div
                                    className="cursor-pointer relative overflow-hidden w-[40px] h-[50px]"
                                    onClick={() => GetUserProfile(member.id)}
                                >
                                    <LayoutAvatarImageView
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    {membersData.admin &&
                                        ...
                                        member.id !== GetSessionDataManager().userId && (
                                            <Flex alignItems="center">
                                                <div
                ...
                <Flex alignItems="center" gap={1} justifyContent="between">
                    <Button
                        disabled={pageId <= 0}
                        onClick={(event) => setPageId((prevValue) => Math.max(0, prevValue - 1))}
                    >
                    ...
                    <Text small>
                        {LocalizeText(
                            ...
                            [
                                membersData.totalMembersCount.toString(),
                                (membersData.pageIndex + 1).toString(),
                                totalPages.toString(),
                            ],
                        )}
                    </Text>

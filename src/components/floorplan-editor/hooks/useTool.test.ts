...
describe('useTool', () => {
    it('SET dispatches PAINT_TILE on pointer down at hit tile', () => {
        ...
        const { result } = renderHook(() =>
            useTool(
                withBrush('SET', 3),
                dispatch as React.Dispatch<FloorplanAction>,
                mockProjection({ row: 1, col: 2 }),
            ),
        );
        act(() =>
    ...
    it('UP dispatches ADJUST_HEIGHT delta=+1', () => {
        const dispatch = vi.fn();
        const { result } = renderHook(() =>
            useTool(withBrush('UP'), dispatch as React.Dispatch<FloorplanAction>, mockProjection({ row: 5, col: 6 })),
        );
        act(() =>
            result.current.onPointerDown({
                clientX: 0,
    ...
    it('DOWN dispatches ADJUST_HEIGHT delta=-1', () => {
        const dispatch = vi.fn();
        const { result } = renderHook(() =>
            useTool(withBrush('DOWN'), dispatch as React.Dispatch<FloorplanAction>, mockProjection({ row: 1, col: 1 })),
        );
        act(() =>
            result.current.onPointerDown({
                clientX: 0,
    ...
    it('DOOR dispatches SET_DOOR with row→y, col→x', () => {
        const dispatch = vi.fn();
        const { result } = renderHook(() =>
            useTool(withBrush('DOOR'), dispatch as React.Dispatch<FloorplanAction>, mockProjection({ row: 4, col: 7 })),
        );
        act(() =>
            result.current.onPointerDown({
                clientX: 0,
    ...
    it('does nothing when projection returns null', () => {
        const dispatch = vi.fn();
        const { result } = renderHook(() =>
            useTool(withBrush('SET'), dispatch as React.Dispatch<FloorplanAction>, mockProjection(null)),
        );
        act(() =>
            result.current.onPointerDown({
                clientX: 0,
    ...
    it('PAINT continues on pointer move when dragging', () => {
        ...
        const { result } = renderHook(() =>
            useTool(withBrush('SET', 0), dispatch as React.Dispatch<FloorplanAction>, projection),
        );
        act(() =>
            result.current.onPointerDown({
                clientX: 0,
    ...
    it('PAINT does not re-dispatch on move within same tile', () => {
        ...
        const { result } = renderHook(() =>
            useTool(withBrush('SET'), dispatch as React.Dispatch<FloorplanAction>, projection),
        );
        act(() =>
            result.current.onPointerDown({
                clientX: 0,

...
export const CatalogPurchaseWidgetView: FC<CatalogPurchaseWidgetViewProps> = (props) => {
    ...
    const [catalogSkipPurchaseConfirmation, setCatalogSkipPurchaseConfirmation] = useLocalStorage(
    ...
    const {
        ...
        getBuilderFurniPlaceableStatus = null,
    ...
    const isLimitedSoldOut = useMemo(() => {
        ...
        if (purchaseOptions.extraParamRequired && (!purchaseOptions.extraData || !purchaseOptions.extraData.length))
            return false;
    ...
    const purchase = (isGift: boolean = false) => {
        ...
        if (isGift) {
            DispatchUiEvent(
                new CatalogInitGiftEvent(currentOffer.page.pageId, currentOffer.offerId, purchaseOptions.extraData),
            );
    ...
    const isBuildersClubPlaceable =
        ...
        (currentOffer.product.productType === ProductTypeEnum.FLOOR ||
            currentOffer.product.productType === ProductTypeEnum.WALL);
    const builderPlaceableStatus = useMemo(() => {
        if (!isBuildersClubPlaceable || !getBuilderFurniPlaceableStatus || !currentOffer)
    ...
    const PurchaseButton = () => {
        ...
        switch (purchaseState) {
            case CatalogPurchaseState.CONFIRM:
                return (
                    <Button
                        classNames={[...swfButtonClassNames, 'nitro-catalog-swf-confirm-button']}
                        variant="success"
                        onClick={(event) => purchase()}
                    >

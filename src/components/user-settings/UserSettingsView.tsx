...
import { FaPalette, FaCog, FaVolumeDown, FaVolumeMute, FaVolumeUp } from 'react-icons/fa';
...
export const UserSettingsView: FC<{}> = props =>
{
    ...
    const [ activeTab, setActiveTab ] = useState<'general' | 'interface'>('general');
    ...
    return (
        <NitroCardView className="user-settings-window w-[380px]" theme="primary-slim" uniqueKey="user-settings">
            <NitroCardHeaderView headerText={ LocalizeText('widget.memenu.settings.title') } onCloseClick={ event => processAction('close_view') } />
            <NitroCardTabsView>
                <NitroCardTabsItemView isActive={ activeTab === 'general' } onClick={ () => setActiveTab('general') }>
                    <div className="flex items-center gap-1">
                        <FaCog size={ 11 } />
                        { LocalizeText('widget.memenu.settings.tab.general') }
                    </div>
                </NitroCardTabsItemView>
                <NitroCardTabsItemView isActive={ activeTab === 'interface' } onClick={ () => setActiveTab('interface') }>
                    <div className="flex items-center gap-1">
                        <FaPalette size={ 11 } />
                        { LocalizeText('widget.memenu.settings.tab.interface') }
                    </div>
                </NitroCardTabsItemView>
            </NitroCardTabsView>

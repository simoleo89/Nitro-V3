...
export const WiredActionBotChangeFigureView: FC = (props) => {
    ...
    useEffect(() => {
        ...
        setBotSource(
            trigger.intData.length > 0
                ? normalizeBotSource(trigger.intData[0], nextBotName.length > 0)
                : normalizeBotSource(-1, nextBotName.length > 0),
        );
    }, [trigger]);
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-center">
                ...
                <Button onClick={(event) => setFigure(GetSessionDataManager().figure)}>
                    {LocalizeText('wiredfurni.params.capture.figure')}
                </Button>
            </div>
        </WiredActionBaseView>
    );

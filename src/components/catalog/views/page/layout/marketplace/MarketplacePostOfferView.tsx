...
export const MarketplacePostOfferView: FC = (props) => {
    ...
    const getFurniTitle = item
        ? LocalizeText(item.isWallItem ? 'wallItem.name.' + item.type : 'roomItem.name.' + item.type)
        : '';
    const getFurniDescription = item
        ? LocalizeText(item.isWallItem ? 'wallItem.desc.' + item.type : 'roomItem.desc.' + item.type)
        : '';
    ...
    const postItem = () => {
        ...
        showConfirm(
            LocalizeText(
                ...
                ['furniname', 'price'],
                [getFurniTitle, askingPrice.toString()],
            ),
            () => {
    ...
    return (
        <NitroCardView className="nitro-catalog-layout-marketplace-post-offer" theme="primary-slim">
            <NitroCardHeaderView
                headerText={LocalizeText('inventory.marketplace.make_offer.title')}
                onCloseClick={(event) => setItem(null)}
            />
                ...
                <Grid fullHeight>
                    ...
                    <Column justifyContent="between" overflow="hidden" size={8}>
                        ...
                        <Column overflow="auto">
                            <Text italics>
                                {LocalizeText(
                                    'inventory.marketplace.make_offer.expiration_info',
                                    ['time'],
                                    [marketplaceConfiguration.offerTime.toString()],
                                )}
                            </Text>

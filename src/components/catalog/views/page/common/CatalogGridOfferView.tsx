...
export const CatalogGridOfferView: FC<CatalogGridOfferViewProps> = (props) => {
        ...
        if (product.productType === ProductTypeEnum.FLOOR || product.productType === ProductTypeEnum.WALL) {
            ...
            if (className?.length) {
                ...
                if (product.productType === ProductTypeEnum.WALL && product.extraParam?.length) {
                    ...
                    product.productType === ProductTypeEnum.FLOOR &&
                    product.furnitureData?.hasIndexedColor &&
                    product.furnitureData.colorIndex > 0
                ) {
    ...
    const prices = useMemo(() => {
        ...
        if (offer.priceInActivityPoints > 0)
            values.push({ amount: offer.priceInActivityPoints, type: offer.activityPointType });
    ...
    return (
        ...
        >
            ...
            {prices.length > 0 && (
                <span
                    className={`nitro-catalog-classic-grid-price ${prices.length > 1 ? 'is-multi-price' : 'is-single-price'}`}
                >
            ...
            >
                <FaHeart
                    className={`text-[10px] drop-shadow transition-colors duration-100 ${isFav ? 'text-danger' : 'text-muted hover:text-danger'}`}
                />
            </div>
        </LayoutGridItem>
    );

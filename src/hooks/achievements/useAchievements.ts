...
const useAchievementsState = () => {
    ...
    const getTotalUnseen = useMemo(() => {
        ...
        achievementCategories.forEach(
            (category) => (unseen += AchievementUtilities.getAchievementCategoryTotalUnseen(category)),
        );
    ...
    useMessageEvent<AchievementEvent>(AchievementEvent, (event) => {
        ...
        setAchievementCategories((prevValue) => {
            ...
            if (categoryIndex === -1) {
                ...
                const achievementIndex = newAchievements.findIndex(
                    (existing) => existing.achievementId === achievement.achievementId,
                );

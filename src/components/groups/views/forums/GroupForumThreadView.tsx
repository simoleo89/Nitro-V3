...
export const GroupForumThreadView: FC<GroupForumThreadViewProps> = (props) => {
    ...
    useMessageEvent<ThreadMessagesMessageEvent>(ThreadMessagesMessageEvent, (event) => {
        ...
        if (parser.messages.length > 0) {
            ...
            SendMessageComposer(
                new UpdateForumReadMarkerMessageComposer(
    ...
    const togglePinThread = useCallback(() => {
        ...
        SendMessageComposer(
            new UpdateThreadMessageComposer(effectiveGroupId, threadId, threadInfo.isLocked, !threadInfo.isPinned),
        );
    ...
    const hideMessage = useCallback(
        (messageId: number) => {
            SendMessageComposer(
                new ModerateMessageMessageComposer(effectiveGroupId, threadId, messageId, STATE_HIDDEN_BY_ADMIN),
            );
        },
        [effectiveGroupId, threadId],
    );
    ...
    const restoreMessage = useCallback(
        (messageId: number) => {
            SendMessageComposer(
                new ModerateMessageMessageComposer(effectiveGroupId, threadId, messageId, STATE_VISIBLE),
            );
        },
        [effectiveGroupId, threadId],
    );
            ...
            <Flex className="bg-muted p-2 border-b" gap={2} alignItems="center" justifyContent="between">
                ...
                {canModerate && (
                    <Flex gap={1}>
                        ...
                        >
                            {threadInfo?.isPinned
                                ? LocalizeText('groupforum.thread.unpin')
                                : LocalizeText('groupforum.thread.pin')}
                        </Button>
                        <Button
                            variant="outline-secondary"
                            className="btn-sm rounded-md text-white bg-[#5cb85c] border-[#5cb85c] [box-shadow:inset_0_2px_#ffffff26,inset_0_-2px_#0000001a,0_1px_#0000001a] hover:text-white hover:bg-[#4cae4c] hover:border-[#47a447]"
                        ...
                        >
                            {isLocked
                                ? LocalizeText('groupforum.thread.unlock')
                                : LocalizeText('groupforum.thread.lock')}
                        </Button>
                        <Button
                            variant="outline-secondary"
                            className="btn-sm rounded-md text-white bg-[#5cb85c] border-[#5cb85c] [box-shadow:inset_0_2px_#ffffff26,inset_0_-2px_#0000001a,0_1px_#0000001a] hover:text-white hover:bg-[#4cae4c] hover:border-[#47a447]"
                    ...
                    if (stateText && !canModerate) {
                        return (
                            <Flex
                                ...
                                className="p-2 border-b bg-danger bg-opacity-10"
                                alignItems="center"
                            >
                                ...
                                <Flex justifyContent="between" alignItems="center">
                                    ...
                                    {canModerate && message.state !== STATE_NORMAL && (
                                        <Flex gap={1}>
                                            ...
                                            <Text
                                                ...
                                                underline
                                                variant="primary"
                                                onClick={() => restoreMessage(message.messageId)}
                                            >
                                    ...
                                    {canModerate && message.state === STATE_NORMAL && (
                                        <Text
                                            ...
                                            underline
                                            variant="danger"
                                            onClick={() => hideMessage(message.messageId)}
                                        >
                ...
                {messages.length < totalMessages && (
                    <Flex justifyContent="center" className="p-2">
                        <Text
                            ...
                            onClick={() => {
                                SendMessageComposer(
                                    new GetMessagesMessageComposer(
                                        effectiveGroupId,
                                        threadId,
                                        messages.length,
                                        MESSAGES_PER_PAGE,
            ...
            {canPost && !isLocked && (
                <Flex className="p-2 border-t bg-light" gap={2}>
                    ...
                    <Button
                        ...
                        onClick={sendReply}
                        disabled={replyText.trim().length < 10 || isSubmitting}
                    >

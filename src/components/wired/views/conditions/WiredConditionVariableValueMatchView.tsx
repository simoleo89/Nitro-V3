...
export const WiredConditionVariableValueMatchView: FC = () => {
    ...
    const variableEntries = useMemo(
        () => buildWiredVariablePickerEntries(targetType, 'change-reference', targetDefinitions),
        [targetDefinitions, targetType],
    );
    ...
    const resolvedVariableEntries = useMemo(() => {
        ...
        if (flattenWiredVariablePickerEntries(variableEntries).some((entry) => entry.token === variableToken))
    ...
    useEffect(() => {
        ...
        setQuantifier(
            trigger.intData.length > 9
                ? trigger.intData[9] === QUANTIFIER_ANY
                    ? QUANTIFIER_ANY
                    : QUANTIFIER_ALL
                : QUANTIFIER_ALL,
        );

...
describe('FloorplanTile', () => {
    it('renders nothing for blocked tile by default', () => {
        const { container } = render(
            svg(<FloorplanTile row={0} col={0} tile={{ h: 0, blocked: true }} selected={false} isDoor={false} />),
        );
    ...
    it('renders a polygon for non-blocked tile', () => {
        const { container } = render(
            svg(<FloorplanTile row={0} col={0} tile={{ h: 3, blocked: false }} selected={false} isDoor={false} />),
        );
    ...
    it('renders a door marker when isDoor=true', () => {
        const { container } = render(
            svg(<FloorplanTile row={1} col={1} tile={{ h: 0, blocked: false }} selected={false} isDoor={true} />),
        );
    ...
    it('applies selection ring when selected', () => {
        const { container } = render(
            svg(<FloorplanTile row={0} col={0} tile={{ h: 0, blocked: false }} selected={true} isDoor={false} />),
        );

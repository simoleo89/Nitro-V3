...
export const HousekeepingUsersTab: FC = () => {
    ...
    const showSuggestionPanel =
        isFocused && (userSuggestions.length > 0 || (recentUsers.length > 0 && query.trim().length < 2));
    ...
    const runBulkWithGate = (kind: 'ban' | 'kick' | 'mute', actionLabel: string, runner: () => void) => {
        ...
        if (selectedUserIds.length >= BULK_CONFIRM_THRESHOLD) {
            confirm(
                LocalizeText(
                    'housekeeping.bulk.confirm',
                    ['action', 'count'],
                    [actionLabel, String(selectedUserIds.length)],
                ),
                runner,
            );
    ...
    const bulkKick = () =>
        runBulkWithGate('kick', LocalizeText('housekeeping.action.kick'), () =>
        ...
        <div className="flex flex-col gap-2">
                                  ...
                                  return (
                                      ...
                                      >
                                          <input
                                              ...
                                              title={
                                                  isChecked
                                                      ...
                                                      : LocalizeText('housekeeping.bulk.apply')
                                              }
                                              className="shrink-0"
                                          />
                                          <button
                                          ...
                                          >
                                              <FaCircle
                                                  size={6}
                                                  className={entry.online ? 'text-emerald-500' : 'text-zinc-400'}
                                              />
                ...
                <div className="relative overflow-hidden rounded-lg border border-sky-200 bg-gradient-to-br from-sky-50 via-white to-emerald-50 p-3 shadow-sm">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="grow min-w-0">
                            ...
                            <div className="text-xs text-zinc-600 truncate mt-0.5 italic">
                                {selectedUser.motto || '—'}
                            </div>
                            <div className="grid grid-cols-3 gap-1 text-[10px] mt-2">
                                ...
                                >
                                    ...
                                    <span className="tabular-nums font-semibold text-amber-800">
                                        {selectedUser.creditsBalance.toLocaleString()}
                                    </span>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <span className="tabular-nums font-semibold text-orange-800">
                                        {selectedUser.ducketsBalance.toLocaleString()}
                                    </span>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <span className="tabular-nums font-semibold text-sky-800">
                                        {selectedUser.diamondsBalance.toLocaleString()}
                                    </span>
                                </div>
                            </div>
                            {selectedUser.email && (
                                <div
                                    className="flex items-center gap-1 text-[10px] text-zinc-500 mt-1.5 truncate"
            ...
            {selectedUser && selectedUser.online && (
                <div className="rounded-md border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-1.5 flex items-center gap-1 flex-wrap shadow-sm">
                    ...
                    <Button
                        ...
                        onClick={() => kickFromCurrentRoom(selectedUser.id)}
                    >
                    ...
                    <Button
                        ...
                        variant="warning"
                        disabled={isActionPending}
                        onClick={() => muteInCurrentRoom(selectedUser.id, 2)}
                    >
                    ...
                    <Button
                        ...
                        variant="warning"
                        disabled={isActionPending}
                        onClick={() => muteInCurrentRoom(selectedUser.id, 10)}
                    >
                    ...
                    <Button
                        ...
                        variant="danger"
                        disabled={isActionPending}
                        onClick={() => banFromCurrentRoom(selectedUser.id, 'hour')}
                    >
                    ...
                    <Button
                        ...
                        variant="danger"
                        disabled={isActionPending}
                        onClick={() => banFromCurrentRoom(selectedUser.id, 'day')}
                    >
            ...
            <div className="flex flex-col gap-1.5 rounded-md border border-zinc-200 bg-zinc-50/50 p-2">
                <div className="flex items-center gap-1.5">
                    <label className="text-[10px] uppercase tracking-wider font-semibold opacity-60 shrink-0">
                        Template
                    </label>
                    <select
                ...
                <label className="text-[10px] uppercase tracking-wider font-semibold opacity-60">
                    {LocalizeText('housekeeping.field.reason')}
                </label>
                <textarea
            ...
            <label className="text-[10px] uppercase tracking-wider font-semibold opacity-60 -mb-0.5">
                {LocalizeText('housekeeping.field.duration')}
            </label>
            <div className="grid grid-cols-2 gap-1.5">
                ...
                <Button
                    variant="warning"
                    disabled={disableActions}
                    onClick={() => kickUser(selectedUser.id, reasonOrDefault)}
                >
                    <FaUserSlash size={10} />
                    <span className="ml-1">{LocalizeText('housekeeping.action.kick')}</span>
                </Button>
                <Button
                    variant="danger"
                    disabled={disableActions || !selectedUser?.isBanned}

...
const normalizeSubvariables = (value?: number[] | null) => {
    ...
    return [
        ...new Set(
            value.filter(
                (subvariable) =>
                    Number.isInteger(subvariable) && subvariable >= SUB_CURRENT_LEVEL && subvariable <= SUB_MAX_LEVEL,
            ),
        ),
    ];
};
...
export const WiredExtraVariableLevelUpSystemView: FC = () => {
    ...
    useEffect(() => {
        ...
        setStepSizeInput(
            normalizeNonNegativeInt(editorData.stepSize ?? DEFAULT_STEP_SIZE, DEFAULT_STEP_SIZE).toString(),
        );
        setMaxLevelInput(normalizePositiveInt(editorData.maxLevel ?? DEFAULT_MAX_LEVEL, DEFAULT_MAX_LEVEL).toString());
        setFirstLevelXpInput(
            normalizeNonNegativeInt(
                editorData.firstLevelXp ?? DEFAULT_FIRST_LEVEL_XP,
                DEFAULT_FIRST_LEVEL_XP,
            ).toString(),
        );
        setIncreaseFactorInput(
            normalizeNonNegativeInt(
                editorData.increaseFactor ?? DEFAULT_INCREASE_FACTOR,
    ...
    const normalizedStepSize = useMemo(
        () => normalizeNonNegativeInt(parseIntInput(stepSizeInput, DEFAULT_STEP_SIZE), DEFAULT_STEP_SIZE),
        [stepSizeInput],
    );
    const normalizedMaxLevel = useMemo(
        () => normalizePositiveInt(parseIntInput(maxLevelInput, DEFAULT_MAX_LEVEL), DEFAULT_MAX_LEVEL),
        [maxLevelInput],
    );
    ...
    const normalizedIncreaseFactor = useMemo(
        () =>
            normalizeNonNegativeInt(
                parseIntInput(increaseFactorInput, DEFAULT_INCREASE_FACTOR),
                DEFAULT_INCREASE_FACTOR,
            ),
        [increaseFactorInput],
    );
    ...
    const previewEntries = useMemo(
        () =>
            buildPreviewEntries(
                ...
                normalizedStepSize,
                normalizedMaxLevel,
                normalizedFirstLevelXp,
                normalizedIncreaseFactor,
    ...
    const interpolationPlaceholder = useMemo(() => {
        ...
        if (!localizedText || localizedText === 'wiredfurni.params.levelup.interpolation_placeholder')
        ...
        >
            <div className="nitro-wired__levelup">
                <div className="nitro-wired__levelup-section">
                    <button
                        type="button"
                        className="nitro-wired__levelup-section-header"
                        onClick={() => setIsModeSectionOpen((value) => !value)}
                    >
                    ...
                    {isModeSectionOpen && (
                        <div className="nitro-wired__levelup-section-body">
                            <div
                                className={`nitro-wired__levelup-mode-block ${mode === MODE_LINEAR ? 'is-active' : 'is-inactive'}`}
                            >
                            ...
                            <div
                                className={`nitro-wired__levelup-mode-block ${mode === MODE_EXPONENTIAL ? 'is-active' : 'is-inactive'}`}
                            >
                                <label className="nitro-wired__levelup-mode-label">
                                    <input
                                    ...
                                    />
                                    <Text>
                                        {localizeOrFallback('wiredfurni.params.levelup.mode.2', 'Esponenziale')}
                                    </Text>
                                </label>
                                <div className="nitro-wired__levelup-fields">
                                    <div className="nitro-wired__levelup-field-row">
                            ...
                            <div
                                className={`nitro-wired__levelup-mode-block ${mode === MODE_MANUAL ? 'is-active' : 'is-inactive'}`}
                            >
                                <label className="nitro-wired__levelup-mode-label">
                                    <input
                ...
                <div className="nitro-wired__levelup-section">
                    <button
                        type="button"
                        className="nitro-wired__levelup-section-header"
                        onClick={() => setIsPreviewSectionOpen((value) => !value)}
                    >
                        <Text bold>{LocalizeText('wiredfurni.params.levelup.preview')}</Text>
                        <span className={`nitro-wired__levelup-chevron ${isPreviewSectionOpen ? 'is-open' : ''}`}>
                            ^
                        </span>
                    </button>
                ...
                <div className="nitro-wired__levelup-section">
                    <button
                        type="button"
                        className="nitro-wired__levelup-section-header"
                        onClick={() => setIsSubvariablesSectionOpen((value) => !value)}
                    >
                        <Text bold>{LocalizeText('wiredfurni.params.create_subvariables')}</Text>
                        <span className={`nitro-wired__levelup-chevron ${isSubvariablesSectionOpen ? 'is-open' : ''}`}>
                            ^
                        </span>
                    </button>
                            ...
                            {subvariableOptions.map((subvariable) => (
                                <div key={subvariable.key} className="nitro-wired__levelup-subvariable-row">
                                    <label className="nitro-wired__levelup-subvariable-label">
                                        ...
                                        />
                                        <Text>
                                            {LocalizeText(`wiredfurni.params.levelup.subvariable.${subvariable.key}`)}
                                        </Text>
                                    </label>
                                    <input
                                        className="nitro-wired__levelup-subvariable-token"
                                        readOnly
                                        tabIndex={-1}
                                        type="text"
                                        value={subvariable.suffix}
                                    />
                                </div>
                            ))}
                        </div>

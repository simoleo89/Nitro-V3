...
export const CatalogLayoutVipGiftsView: FC<CatalogLayoutProps> = (props) => {
    ...
    const giftsAvailable = useCallback(() => {
        ...
        if (clubGifts.daysUntilNextGift > 0)
            return LocalizeText(
                'catalog.club_gift.days_until_next',
                ['days'],
                [clubGifts.daysUntilNextGift.toString()],
            );
    ...
    const sortGifts = useMemo(() => {
        ...
        return [...clubGifts.offers].sort(
            (a, b) =>
                clubGifts.getOfferExtraData(a.offerId).daysRequired -
                clubGifts.getOfferExtraData(b.offerId).daysRequired,
        );
    }, [clubGifts]);
                ...
                {clubGifts &&
                    ...
                    sortGifts.map((offer) => (
                        <VipGiftItem
                            ...
                            isAvailable={
                                clubGifts.getOfferExtraData(offer.offerId).isSelectable && clubGifts.giftsAvailable > 0
                            }
                            offer={offer}
                            onSelect={selectGift}
                        />

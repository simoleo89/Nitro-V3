...
export const WiredConditionFurniIsOfTypeView: FC<WiredConditionFurniIsOfTypeViewProps> = ({ negative = false }) => {
    ...
    const switchSelection = useCallback(
        (mode: SelectionMode) => {
            ...
            const canEditSecondary =
                matchSource === SOURCE_SECONDARY_SELECTED || compareSource === SOURCE_SECONDARY_SELECTED;
    ...
    useEffect(() => {
        ...
        const nextMatchSource =
            trigger.intData.length >= 1 ? trigger.intData[0] : nextPrimaryIds.length ? SOURCE_SELECTED : SOURCE_TRIGGER;
        const nextCompareSource =
            trigger.intData.length >= 2
                ? trigger.intData[1]
                : nextSecondaryIds.length
    ...
    const quantifierKeyPrefix = negative
        ? 'wiredfurni.params.quantifier.furni.neg'
        : 'wiredfurni.params.quantifier.furni';

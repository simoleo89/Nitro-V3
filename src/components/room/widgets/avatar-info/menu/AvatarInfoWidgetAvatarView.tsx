...
export const AvatarInfoWidgetAvatarView: FC<AvatarInfoWidgetAvatarViewProps> = (props) => {
    ...
    const moderateMenuHasContent = useMemo(() => {
        return (
            avatarInfo.canBeKicked ||
            avatarInfo.canBeBanned ||
            avatarInfo.canBeMuted ||
            isShowGiveRights ||
            isShowRemoveRights
        );
    ...
    const processAction = (name: string) => {
        ...
        if (name) {
            switch (name) {
                ...
                case 'rship_heart':
                    SendMessageComposer(
                        new SetRelationshipStatusComposer(avatarInfo.webID, MessengerFriend.RELATIONSHIP_HEART),
                    );
                    break;
                case 'rship_smile':
                    SendMessageComposer(
                        new SetRelationshipStatusComposer(avatarInfo.webID, MessengerFriend.RELATIONSHIP_SMILE),
                    );
                    break;
                case 'rship_bobba':
                    SendMessageComposer(
                        new SetRelationshipStatusComposer(avatarInfo.webID, MessengerFriend.RELATIONSHIP_BOBBA),
                    );
                    break;
                case 'rship_none':
                    SendMessageComposer(
                        new SetRelationshipStatusComposer(avatarInfo.webID, MessengerFriend.RELATIONSHIP_NONE),
                    );
                    break;
            }
        }
    ...
    return (
        ...
        >
            <ContextMenuHeaderView
                ...
                dangerouslySetInnerHTML={{ __html: `${avatarInfo.name}` }}
            ></ContextMenuHeaderView>
            {mode === MODE_NORMAL && (
                <>

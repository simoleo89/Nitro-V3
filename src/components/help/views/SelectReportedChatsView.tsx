            ...
            case ReportType.EMERGENCY:
                return chatHistory.filter(
                    (chat) =>
                        ...
                        chat.webId === activeReport.reportedUserId &&
                        chat.entityType === RoomObjectType.USER,
                );
            case ReportType.IM:
                return messengerHistory.filter(
                    (chat) => chat.webId === activeReport.reportedUserId && chat.type === ChatEntryType.TYPE_IM,
                );
        }
                    ...
                    <AutoGrid columnCount={1} columnMinHeight={25} gap={1} overflow="auto">
                        {userChats.map((chat, index) => {
                            return (
                                <LayoutGridItem
                                    key={chat.id}
                                    itemActive={selectedChats.indexOf(chat) >= 0}
                                    onClick={(event) => selectChat(chat)}
                                >

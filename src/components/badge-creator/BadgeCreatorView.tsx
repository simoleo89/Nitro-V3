...
export const BadgeCreatorView: FC = () => {
    ...
    const handleSave = useCallback(async () => {
        ...
        if (!editingBadgeId && !canCreateMore) {
            setError(
                t('badgecreator.error.limit', 'You already have %max% custom badges.', ['max'], [String(maxBadges)]),
            );
            return;
        }
    ...
    return (
        <NitroCardView
            className="nitro-badge-creator w-[760px] h-[680px]"
            isResizable={false}
            theme="primary-slim"
            uniqueKey="badge-creator"
        >
            <NitroCardHeaderView
                headerText={t('badgecreator.title', 'Badge Creator')}
            ...
            <NitroCardContentView className="text-black">
                <Flex gap={2} className="badge-creator-main">
                    <Column gap={2}>
                        ...
                        <Flex gap={1} className="badge-creator-tools">
                            ...
                            <Button onClick={() => setShowGrid((g) => !g)} variant="primary">
                                {showGrid
                                    ? t('badgecreator.tool.gridoff', 'Grid off')
                                    : t('badgecreator.tool.gridon', 'Grid on')}
                            </Button>
                        </Flex>
                    </Column>
                    <Column gap={2} className="badge-creator-side" style={{ minWidth: 220 }}>
                        <div>
                            ...
                            <div
                                className="badge-creator-palette"
                                style={{
                                    display: 'grid',
                ...
                <Column gap={1} className="badge-creator-list" style={{ marginTop: 8 }}>
                    <Text bold variant="black">
                        {t(
                            'badgecreator.list.title',
                            'Your custom badges (%count%/%max%)',
                            ['count', 'max'],
                            [String(badges?.length ?? 0), String(maxBadges)],
                        )}
                    </Text>
                    {badges === null && <Text variant="black">{t('badgecreator.list.loading', 'Loading…')}</Text>}
                    {badges !== null && !badges.length && (
                        <Text variant="black">{t('badgecreator.list.empty', "You haven't made any badges yet.")}</Text>
                    )}
                    {badges !== null &&
                        badges.map((badge) => (
                            <Flex
                                ...
                                alignItems="center"
                                gap={2}
                                style={{ padding: 4, borderTop: '1px solid #ccc' }}
                            >
                                <img
                                    src={badge.url}

...
export const App: FC<{}> = (props) => {
    ...
    const [isEnteringHotel, setIsEnteringHotel] = useState(
        () => !!window.NitroConfig?.['sso.ticket'] || hasRememberLogin(),
    );
    ...
    const tryRememberLogin = useCallback(async (): Promise<string> => {
        ...
        try {
            const rawEndpoint = GetConfiguration().getValue<string>(
                'login.remember.endpoint',
                '${api.url}/api/auth/remember',
            );
            ...
            if (response.ok && ssoTicket) {
                ...
                StoreRememberLoginFromPayload(
                    payload,
                    typeof payload.username === 'string' ? payload.username : remembered.username,
                    ssoTicket,
                );
                return ssoTicket;
            }
    ...
    const startRenderer = useCallback((width: number, height: number) => {
        ...
        const useBackBuffer =
            rawUseBackBuffer === undefined ? true : rawUseBackBuffer === true || rawUseBackBuffer === 'true';
    ...
    const startWarmup = useCallback(
        (width: number, height: number) => {
            ...
            warmupPromiseRef.current = (async () => {
                ...
                startRenderer(width, height).catch((error) =>
                    NitroLogger.error('[LoginScreen] Renderer warmup failed', error),
                );

                const interpolate = (value: string) => GetConfiguration().interpolate(value);
                const assetUrls = asStringArray(GetConfiguration().getValue<unknown>('preload.assets.urls')).map(
                    interpolate,
                );
                const gamedataUrls = [
                ...
                const loginImages =
                    (GetConfiguration().getValue<Record<string, unknown>>('loginview', {})?.images as Record<
                        string,
                        string
                    >) ?? {};
                ...
                const warmupTasks: { promise: Promise<any>; label: string }[] = [
                    {
                        promise: GetAssetManager().downloadAssets(assetUrls),
                        label: taskLabel('loading.task.assets', 'Loading game assets...'),
                    },
                    {
                        promise: GetLocalizationManager().init(),
                        label: taskLabel('loading.task.localization', 'Loading translations...'),
                    },
                    {
                        promise: GetAvatarRenderManager().init(),
                        label: taskLabel('loading.task.avatar', 'Loading wardrobe...'),
                    },
                ...
                await Promise.all(
                    warmupTasks.map((t) =>
                        t.promise.then((value) => {
                            warmupDone++;
                            bumpProgress(
                                warmupStart + Math.round((warmupSpan * warmupDone) / warmupTasks.length),
                                t.label,
                            );
                            return value;
                        }),
        ...
        const prepare = async (width: number, height: number) => {
            ...
            try {
                ...
                if (!ssoTicket || ssoTicket === '') {
                    ...
                    const rawLoginEnabled = GetConfiguration().getValue<unknown>('login.screen.enabled', false);
                    const loginScreenEnabled =
                        rawLoginEnabled === true || rawLoginEnabled === 'true' || rawLoginEnabled === 1;
                    ...
                    if (configInitError) {
                        NitroLogger.error(
                            '[LoginScreen] Failed to load renderer-config.json — cannot resolve login.screen.enabled',
                            configInitError,
                        );
                    }
                    ...
                    if (loginScreenEnabled) {
                        ...
                        if (rememberedSsoTicket) {
                            ...
                            setIsReady(false);
                            setShowLogin(true);
                            startWarmup(width, height).catch((error) =>
                                NitroLogger.error('[LoginScreen] Warmup failed', error),
                            );
                            return;
                        }
                    } else {
                ...
                if (!bootstrapDoneRef.current) {
                    ...
                    if (LegacyExternalInterface.available)
                        LegacyExternalInterface.call('legacyTrack', 'authentication', 'authok', []);
                ...
                const rotateMinutes = Math.max(
                    1,
                    Number(GetConfiguration().getValue<unknown>('login.remember.rotate.interval.minutes', 15)) || 15,
                );
                if (GetRememberLogin()?.token?.length)
                    rememberRotateIntervalRef.current = window.setInterval(
                        () => rotateRememberLogin(),
    ...
    return (
        <Base
            fit
            overflow="hidden"
            className={`nitro-app-root ${!(window.devicePixelRatio % 1) ? 'image-rendering-pixelated' : ''}`}
        >
            {!isReady && !showLogin && (
                <LoadingView

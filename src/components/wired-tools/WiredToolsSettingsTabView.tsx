...
const normalizeAccessMask = (mask: number): number =>
    (mask & WIRED_ACCESS_GROUP_MEMBERS) !== 0 ? mask | WIRED_ACCESS_GROUP_ADMINS : mask;
...
export const WiredToolsSettingsTabView: FC<{}> = () => {
    ...
    const updateModifyMask = (bit: number) => {
        ...
        const nextInspectMask = normalizeAccessMask(
            enabledModifyBit ? roomSettings.inspectMask | bit : roomSettings.inspectMask,
        );
    ...
    const renderAccessOption = (option: RoomAccessOption, mask: number, onToggle: (bit: number) => void) => {
        ...
        return (
            <label
                key={option.label}
                className={`flex items-center gap-2 text-[12px] ${disabled ? 'text-[#8c877d]' : 'text-[#222]'}`}
            >
                <input
                    checked={checked}
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    ...
                    {modifyOptions.map((option) =>
                        renderAccessOption(option, roomSettings.modifyMask, updateModifyMask),
                    )}
                </div>
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>
                    ...
                    {inspectOptions.map((option) =>
                        renderAccessOption(option, roomSettings.inspectMask, updateInspectMask),
                    )}
                </div>
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>
                ...
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    ...
                    <label className="flex items-center gap-2 text-[12px] text-[#222]">
                        <input
                            ...
                            type="checkbox"
                            onChange={(event) =>
                                updateAccountPreferences({ showSystemNotifications: event.target.checked })
                            }
                        />

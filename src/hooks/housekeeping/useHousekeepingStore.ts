...
const useHousekeepingStoreInner = () => {
    ...
    const [activeTab, setActiveTab] = useLocalStorage<HousekeepingTabId>(
        'nitro.housekeeping.last_tab',
    ...
    const [passwordReveal, setPasswordReveal] = useState<{ userId: number; username: string; password: string } | null>(
        null,
    );
    ...
    const [metricsByAction, setMetricsByAction] = useState<Map<string, import('../../api').MetricSample>>(
        () => new Map(),
    );
    ...
    useEffect(() => {
        ...
        const intervalMs =
            typeof configured === 'number' && configured >= AUDIT_POLL_MIN_MS
                ? configured
                : configured === 0
                  ? 0
                  : AUDIT_POLL_DEFAULT_MS;

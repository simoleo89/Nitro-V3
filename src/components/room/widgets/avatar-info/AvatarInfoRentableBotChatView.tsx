...
export const AvatarInfoRentableBotChatView: FC<AvatarInfoRentableBotChatViewProps> = (props) => {
    ...
    const [newText, setNewText] = useState<string>(
        chatEvent.chat === '${bot.skill.chatter.configuration.text.placeholder}' ? '' : chatEvent.chat,
    );
    ...
    const getObjectLocation = useMemo(
        () => GetRoomObjectBounds(GetRoomSession().roomId, chatEvent.objectId, chatEvent.category, 1),
        [chatEvent],
    );
    ...
    const save = () => {
        const chatConfiguration =
            formatChatString(newText) + ';#;' + automaticChat + ';#;' + chatDelay + ';#;' + mixSentences;
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <Flex alignItems="center" gap={1} justifyContent="between">
                            ...
                            <NitroInput
                                type="number"
                                value={chatDelay}
                                onChange={(event) => setChatDelay(event.target.valueAsNumber)}
                            />
                        </Flex>
                    </div>
                    <Flex alignItems="center" gap={1} justifyContent="between">

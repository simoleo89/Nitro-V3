...
export const ChatWidgetWindowView: FC<{}> = () => {
    ...
    const roomChatHistory = useMemo(() => {
        ...
        return chatHistory.filter((chat) => {
            ...
            return `${chat.name} ${chat.message || ''} ${chat.originalMessage || ''} ${chat.translatedMessage || ''}`
                .toLowerCase()
    ...
    return (
        ...
        >
            ...
            <NitroCardContentView className="bg-[#f2f2f2] relative" overflow="hidden">
                <div className="flex items-center gap-2 px-2 py-1 border-b border-black/20 bg-white/40 text-black text-[11px]">
                    <label className="flex items-center gap-1 cursor-pointer select-none">
                        <input
                            checked={hidePets}
                            type="checkbox"
                            onChange={(event) => setHidePets(event.target.checked)}
                        />
                    ...
                    <label className="flex items-center gap-1 cursor-pointer select-none">
                        <input
                            checked={hideAvatars}
                            type="checkbox"
                            onChange={(event) => setHideAvatars(event.target.checked)}
                        />
                    ...
                    >
                        {hideBalloons
                            ? LocalizeText('widget.room.chat.show_balloon')
                            : LocalizeText('widget.room.chat.hide_balloon')}
                    </button>
                    <button
                ...
                <div
                    ref={contentRef}
                    className="h-[calc(100%-31px)] overflow-y-auto px-2 py-1 text-black text-[13px] leading-4"
                    onScroll={onScroll}
                >
                                        ...
                                        {chat.showTranslation && (
                                            <div className="mt-[2px] flex flex-col gap-[2px]">
                                                <div className="flex items-start gap-1 leading-[1.15]">
                                                    <span className="inline-block min-w-[52px] font-bold opacity-75">
                                                        original:
                                                    </span>
                                                    <span
                                                        className={messageClassName}
                                                        dangerouslySetInnerHTML={{
                                                            __html: chat.originalMessage || chat.message || '',
                                                        }}
                                                    />
                                                </div>
                                                <div className="flex items-start gap-1 leading-[1.15]">
                                                    ...
                                                    <span
                                                        className={messageClassName}
                                                        dangerouslySetInnerHTML={{
                                                            __html: chat.translatedMessage || chat.message || '',
                                                        }}
                                                    />
                                                </div>
                                            </div>
                                    ...
                                    <div className="bubble-container relative inline-flex items-start">
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                {chat.showTranslation && (
                                                    <div
                                                        className="mt-[2px] flex flex-col gap-[2px]"
                                                        onClick={onClickChat}
                                                    >
                                                        <div className="flex items-start gap-1 leading-[1.1]">
                                                            <span
                                                                className="inline-block min-w-[52px] font-bold"
                                                                style={{ opacity: 0.75 }}
                                                            >
                                                                original:
                                                            </span>

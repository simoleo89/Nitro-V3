...
export const AvatarEffectsView: FC = () => {
    ...
    return (
        <NitroCardView
            className="nitro-avatar-effects w-[640px] h-[480px]"
            isResizable={false}
            uniqueKey="avatar-effects"
            theme="primary-slim"
        >
            <NitroCardHeaderView
                headerText={LocalizeText('product.type.effect') || 'Avatar effect'}
            ...
            <NitroCardContentView className="flex flex-row gap-3 text-black">
                ...
                <Column overflow="hidden" className="flex-1 min-h-0">
                    ...
                    <div className="flex items-center justify-between px-1 py-1 text-[11px] uppercase tracking-wide text-[#666] border-b border-[#2a2a2a]/10">
                        <span>
                            {filteredEffects.length === effects.length
                                ? `${effects.length} effects`
                                : `${filteredEffects.length} of ${effects.length}`}
                        </span>
                        {selectedId > 0 && (
                            <button
                                type="button"
                    ...
                    <div className="flex-1 min-h-0 overflow-auto">
                        ...
                        {!loadError && !effects.length && (
                            <div className="text-sm px-2 py-3 opacity-70">
                                ...
                                {visibleEffects.map((effect, index) => {
                                    ...
                                    return (
                                        <li key={effect.id}>
                                            ...
                                            >
                                                <span
                                                    className={`font-mono text-xs w-12 text-right shrink-0 ${isSelected ? 'opacity-80' : 'opacity-50'}`}
                                                >
                                                    #{id}
                                                </span>
                                    ...
                                    <li className="px-3 py-2 border-t border-[#2a2a2a]/10 mt-1">
                                        ...
                                        >
                                            ...
                                            <span className="opacity-60 ml-1 font-normal">
                                                ({filteredEffects.length - visibleEffects.length} more)
                                            </span>
                                        </button>
                                    </li>
                                )}

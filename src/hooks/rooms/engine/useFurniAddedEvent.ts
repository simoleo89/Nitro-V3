...
export const useFurniAddedEvent = (isActive: boolean, handler: (event: RoomWidgetUpdateRoomObjectEvent) => void) => {
    useEffect(() => {
        ...
        UI_EVENT_DISPATCHER.addEventListener(
            RoomWidgetUpdateRoomObjectEvent.FURNI_ADDED,
            onRoomWidgetUpdateRoomObjectEvent,
        );

        return () => {
            UI_EVENT_DISPATCHER.removeEventListener(
                RoomWidgetUpdateRoomObjectEvent.FURNI_ADDED,
                onRoomWidgetUpdateRoomObjectEvent,
            );
        };
